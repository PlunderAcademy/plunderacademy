...
export default async function ArticlePage(props: { params: Params }) {
  ...
  // Redirect glossary pages to the new /glossary route
  const glossarySlugs = ['island1-glossary', 'island2-glossary', 'island3-glossary', 'island4-glossary', 'island5-glossary', 'blockchain-glossary'];
  if (glossarySlugs.includes(slug)) {
    // Handle legacy blockchain-glossary redirect
    const targetSlug = slug === 'blockchain-glossary' ? 'island1-glossary' : slug;
    redirect(`/glossary/${targetSlug}`);
  }
