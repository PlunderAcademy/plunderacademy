...
export async function getArticleBySlug(slug: string) {
  ...
  const slugRedirects: Record<string, string> = {
    'blockchain-glossary': 'island1-glossary',
  };
  
  // Redirect old slugs to new ones
  if (slugRedirects[slug]) {
    slug = slugRedirects[slug];
  }
...
export async function getModules(): Promise<ModuleMeta[]> {
  ...
  const moduleMetadata: Record<string, { title: string; description: string; order: number; island: string }> = {
    ...
    'advanced-security': {
      ...
      description: 'Master smart contract security from vulnerability detection to incident response.',
      order: 23,
      island: 'neon'
    }
  };
...
export async function getGlossaries(): Promise<ArticleMeta[]> {
  ...
  // Sort glossaries by island order (island1, island2, island3, island4, island5)
  const glossaryOrder = [
    'island1-glossary',
    'island2-glossary',
    'island3-glossary',
  ...
  return glossaries.sort((a, b) => {
    const aIndex = glossaryOrder.indexOf(a.slug);
    const bIndex = glossaryOrder.indexOf(b.slug);
    
    // If both glossaries are in the order, sort by their position
    if (aIndex !== -1 && bIndex !== -1) {
      return aIndex - bIndex;
    }
    
    // If only one glossary is in the order, prioritize it
    if (aIndex !== -1) return -1;
    if (bIndex !== -1) return 1;
    
    // If neither glossary is in the order, maintain original order
    return 0;
  });
}

export async function getGlossaryBySlug(slug: string) {
  // Handle legacy slug redirects for glossaries
  const slugRedirects: Record<string, string> = {
    'blockchain-glossary': 'island1-glossary',
  };
  ...
  throw new Error(`Glossary with slug "${slug}" not found`);
}
