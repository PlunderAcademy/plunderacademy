...
import { fetchIslands } from "@/lib/content";
import { MapZoom } from "@/components/map-zoom";

function LessonsPage() {
  const islands = Route.useLoaderData();
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-10">
          <div className="text-center space-y-4 mb-8">
            ...
            <p className="mx-auto max-w-2xl text-base md:text-lg text-slate-300">
              Navigate through five legendary islands and discover the treasures
        ...
        <div className="space-y-16">
          <div className="text-center space-y-6">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              Each location holds ancient secrets of blockchain mastery. Start
              your journey and collect treasures of knowledge along the way.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
            {islands.map((island, index) => (
              <Card
                key={island.slug}
                className={`group relative overflow-hidden border-2 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl bg-white dark:bg-slate-900`}
              >
                {/* Island Header */}
                <div className={`relative h-48 overflow-hidden`}>
                  <Image
                    src={`/islands/island${index + 1}/island${
                      index + 1
                    }-map.webp`}
                    alt={island.title}
                    fill
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-2">
                    <div
                      className={`w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm border-2 border-white/50 flex items-center justify-center shadow-xl`}
                    >
                  ...
                  <div className="absolute bottom-4 left-4">
                    <div
                      className={`w-12 h-12 rounded-full bg-gradient-to-br from-white/20 to-white/10 border-2 border-white/50 flex items-center justify-center shadow-xl backdrop-blur-sm`}
                    >
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                        <div
                          className={`bg-gradient-to-r from-${island.color}-400 to-${island.color}-500 h-2 rounded-full w-0 group-hover:w-full transition-all duration-1000 delay-300`}
                        />
                      </div>
                  ...
                  <Button
                    ...
                    size="lg"
                  >
                    <Link to={`/lessons/island${index + 1}` as "/lessons/island1"}>
                ...
                <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                  <div
                    className={`absolute inset-0 rounded-xl bg-gradient-to-br from-${island.color}-500/20 to-${island.color}-600/20 opacity-10`}
                  />
                </div>
              </Card>
            ))}
        ...
        <Card className="border-2 relative overflow-hidden">
          ...
          <CardHeader className="text-center space-y-4 relative">
            ...
            <CardDescription className="text-lg">
              Embark on your legendary journey from blockchain basics to
              advanced EVM development. Your adventure to becoming an EVM master
              starts here!
            </CardDescription>
          </CardHeader>
