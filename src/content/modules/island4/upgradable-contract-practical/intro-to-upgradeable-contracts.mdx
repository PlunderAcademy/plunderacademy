...
contract V1 is UUPSUpgradeable {
    ...
    constructor() {
        owner = msg.sender; // ❌ Sets owner in IMPLEMENTATION storage
    }
}
```
...
contract V1 is Initializable, UUPSUpgradeable {
    ...
    function initialize(address _owner) external initializer {
        owner = _owner; // ✅ Sets owner in PROXY storage
    }
}
```
...
<Callout type="info">
**Heads up:** In the Deployment lesson, you'll create a brand new **Foundry** project (not Hardhat!). If you completed previous practicals, make sure to close that project first and start fresh in a new folder. Each practical module should be its own independent project—and this one uses a different toolchain!
</Callout>
...
**Next up:** We'll dive into the TrainingRegistry contract architecture—the actual code powering this training portal's achievement system. You'll see how EIP-712 vouchers work, how claimant tracking proves you deployed a contract, and how storage layout is structured for safe upgrades.

Ready to deploy the same contract you've been interacting with? Let's go! 🚀
