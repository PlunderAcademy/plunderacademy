...
2. **Maintain storage layout** (append-only, adjust __gap)
...
Claim 5 achievements:
  submitVoucher(1) → 150,000 gas
  submitVoucher(2) → 150,000 gas
  submitVoucher(3) → 150,000 gas
  submitVoucher(4) → 150,000 gas
  submitVoucher(5) → 150,000 gas
  ────────────────────────────
  Total:           750,000 gas
```
...
```
Claim 5 achievements:
  submitVoucherBatch([1,2,3,4,5]) → 250,000 gas
  ────────────────────────────────────────────
  Total:                         250,000 gas
...
5. **Reduce __gap** by slots consumed
    ...
    ERC1155Upgradeable
{
    ...
    // V2 STORAGE (APPENDED)
    // ========================================
    uint256 public batchSubmissionCount;  // Track batch usage
    ...
    uint256[37] private __gap;  // Reduced from 38 (consumed 1 slot for batchSubmissionCount)
}
```

**Key changes from V1:**
1. ✅ V1 storage preserved exactly (lines 30-35)
2. ✅ New variable appended: `batchSubmissionCount` (line 40)
3. ✅ New event: `BatchSubmitted` (line 45)
4. ✅ V1 function unchanged: `submitVoucher()` (lines 70-83)
5. ✅ New function added: `submitVoucherBatch()` (lines 90-133)
6. ✅ __gap reduced: 38 → 37 (line 190)
7. ✅ Still has `_authorizeUpgrade()` (line 187) - CRITICAL!
...
**Expected output:**
```
Running 7 tests for test/Upgrade.t.sol:UpgradeTest
[PASS] testUpgradePreservesState() (gas: 456789)
[PASS] testV2BatchRevertsOnEmptyArray() (gas: 123456)
[PASS] testV2BatchRevertsOnLengthMismatch() (gas: 134567)
[PASS] testV2BatchSkipsCompleted() (gas: 345678)
[PASS] testV2BatchSubmission() (gas: 567890)
[PASS] testV2OldFunctionStillWorks() (gas: 234567)

Test result: ok. 7 passed; 0 failed; finished in 5.67ms
```
...
**Output:**
```
V1 (5 individual): 750,000
V2 (1 batch of 5): 250,000
Savings: 500,000
Savings %: 66

✅ V2 batch saves 66% gas!
```
...
```bash
# V1 layout
forge inspect TrainingRegistryV1 storage-layout --pretty > v1-storage.json

# V2 layout
forge inspect TrainingRegistryV2 storage-layout --pretty > v2-storage.json
...
✅ **Created TrainingRegistryV2** with batch voucher submission  
✅ **Maintained storage layout** (append-only, adjusted __gap)  
✅ **Deployed V2 implementation**  
✅ **Upgraded proxy** from V1 to V2  
✅ **Tested state preservation** (all V1 data intact)  
✅ **Verified gas savings** (66% reduction for batch of 5)  
✅ **Tested upgrade safety** (comprehensive test suite)  
✅ **Understood upgrade pitfalls** (no initializer, preserve storage, keep _authorizeUpgrade)  
...
**Congratulations!** You've mastered upgradeable smart contracts - one of the most advanced topics in Solidity development. This skill is highly valued in the industry and essential for building long-lived DeFi protocols. 🎉🚀
