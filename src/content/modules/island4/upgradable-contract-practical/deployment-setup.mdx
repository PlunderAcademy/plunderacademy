...
**⚠️ Windows Users:** You MUST use WSL (Windows Subsystem for Linux) for Foundry. All commands below should be run in WSL, not PowerShell or CMD.
...
# ├── src/              # Contracts here (empty)
# ├── script/           # Deployment scripts (empty)
# ├── test/             # Tests here (empty)
# ├── lib/              # Dependencies (forge-std included)
# ├── foundry.toml      # Configuration
# └── .gitignore
```

**What was created:**
- Empty `src/`, `script/`, and `test/` directories
...
Update `foundry.toml` for Zilliqa compatibility:
...
# Import development wallet to MetaMask
...
# Check if issuer
cast call $PROXY_ADDRESS "isIssuer(address)" 0xIssuerAddress --rpc-url zilliqaTestnet
```
...
├── src/
│   └── TrainingRegistryV1.sol
├── script/
│   ├── Deploy.s.sol
│   └── Verify.s.sol
├── test/
│   └── TrainingRegistryV1.t.sol  (next lesson!)
├── lib/
│   ├── forge-std/
│   ├── openzeppelin-contracts/
│   └── openzeppelin-contracts-upgradeable/
...
✅ **Compared Foundry vs Hardhat** (Solidity tests, faster compilation)  
✅ **Set up project** with OpenZeppelin upgradeable contracts  
✅ **Added claimant tracking** (4 lines, same as NFT/Staking!)  
✅ **Wrote deployment script** in Solidity (not TypeScript!)  
✅ **Deployed to Zilliqa testnet** with proxy + implementation  
✅ **Verified deployment** on block explorer  
✅ **Used cast** for contract interaction  
...
**Next up:** We'll write comprehensive Solidity tests for TrainingRegistryV1, covering initialization, voucher submission, soulbound transfers, and more. Get ready to test in Solidity! 🧪🚀
