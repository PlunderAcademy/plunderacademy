...
```solidity
// ❌ BAD: Predictable and manipulatable
...
**How it works:**
- Users commit to a future target block (commitment phase)
- Wait for the target block to be produced
- Reveal using the target block's hash (reveal phase)
- Combines user-specific entropy with the target block's hash
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Fixes the outcome the moment the block is produced</td>
    </tr>
    <tr>
      <td><strong>Reveal</strong></td>
      <td>Generate random number using the target block's hash</td>
      <td>Combines user + target block entropy</td>
    </tr>
  </tbody>
</table>
...
**1. Unpredictability**
- The target block's hash is unknown at commit time
...
- Entropy is anchored to the committed target block's hash, which is fixed the moment that block is produced
- The result is the same no matter which block the reveal lands in — revealers can't time their transaction to shop for a better outcome
- Range committed upfront (prevents post-hoc selection)

### Entropy Sources

Our implementation combines two sources:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Different users → different results</td>
    </tr>
    <tr>
...
1. **User Address**: Ensures different users get different results even when committing to the same target block
2. **Target Block Hash**: Unknown at commit time, so no one can predict the outcome — and fixed the instant the target block is mined, so no one can influence the outcome afterwards by choosing when to reveal

Anchoring the entropy to the committed target block is the heart of the design: the random number is fully determined before the reveal transaction is even sent, which means the timing of the reveal cannot change the result.
...
✅ **Configurable random ranges** (1 to 2 billion)  
✅ **Target-block-anchored entropy** for unpredictability  
✅ **Fair distribution** (mathematically proven)  
✅ **Event emission** for transparency  
✅ **Commitment management** (cancel stale commitments)  
✅ **Comprehensive test suite**  
✅ **Zilliqa 2.0 deployment**  
...
3. **Deployment Setup** - **Fresh project setup** + Zilliqa testnet deployment with Hardhat 3
4. **Testing & Verification** - Comprehensive test suite and security analysis

<Callout type="info">
**Heads up:** In Lesson 3, you'll create a brand new Hardhat project for your RNG contract. If you completed previous practicals, make sure to close that project first and start fresh in a new folder. Each practical module should be its own independent project!
</Callout>
