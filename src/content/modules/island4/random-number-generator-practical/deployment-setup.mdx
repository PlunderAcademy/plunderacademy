...
const networkName = process.env.HARDHAT_NETWORK || "localhost";
const rngAddress = process.env.RNG_CONTRACT_ADDRESS as `0x${string}`;
const checkAddress = (process.env.CHECK_ADDRESS || process.env.CLAIMANT_ADDRESS) as `0x${string}`;
...
async function main() {
  ...
  if (currentBlock <= cancelBlock) {
    const blocksToWait = Number(cancelBlock - currentBlock);
...
Transaction hash: 0x...
✅ RNG Contract deployed to: 0x5FbDB2315678afecb367f032d93F642f64180aa3
...
⏰ Status: Waiting for target block
Blocks to wait: 8
Time to wait: ~8 seconds
═══════════════════════════════════════
```

**4. Mine blocks manually to reach target (optional):**

In Terminal 1 (where Hardhat node is running), you can mine blocks:
```javascript
// In another terminal:
npx hardhat console --network localhost

// Then in console:
await network.provider.send("hardhat_mine", ["0xa"]); // Mine 10 blocks
```

Or just wait for 10 seconds (Hardhat automatically mines blocks).
