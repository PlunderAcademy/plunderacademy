...
✅ **Multiple Entropy Sources**: Combines user, block, and validator randomness  
...
- **Block reached**: Current block ≥ target block
...
❌ **Attack**: User only reveals if outcome is favorable
✅ **Prevention**: Range committed upfront, cannot change after seeing outcome
...
✅ **Prevention**: Uses previous block hash (already finalized) + PREVRANDAO (expensive to manipulate)
...
❌ **Attack**: Reveal so late that blockhash() returns 0
✅ **Prevention**: MAX_BLOCKS_DELAY limits commitment lifetime
...
✅ **Security**: Multiple entropy sources, manipulation-resistant  
