...
We will follow the same principles as the `zq2-random-number-commit-reveal` reference implementation: bind commitments to the revealer, enforce time windows, and anchor block entropy to a predetermined block so the outcome is fixed before anyone reveals. Reference implementation: `https://github.com/Plunderswap/zq2-random-number-commit-reveal`. You'll learn how to map outputs fairly, eliminate modulo bias with rejection sampling, and understand where VRF fits in production systems. These patterns apply to any application requiring provable fairness: lotteries, gaming, NFT trait assignment, and winner selection.
...
The two‑phase flow prevents front‑running: commitments are opaque until the reveal window opens. Once all participants have committed, reveals happen simultaneously (or within a narrow window), and the contract combines them to produce the final random value. If you mix in a `blockhash` for extra entropy, anchor it to a **predetermined block** (for example, the first block after the commit window closes): its hash is unknown during commit but fixed the moment that block is produced, so the revealer can't change the outcome by timing their reveal transaction. Never hash in a block chosen at reveal time (such as `blockhash(block.number - 1)`)—that lets the revealer resubmit across different blocks and shop for a favorable result. And `blockhash` alone is insufficient due to validator influence—use it only as an additional entropy source, not the primary one.
...
contract CommitRevealRng {
  ...
  function reveal(uint256 secret, bytes32 salt) external returns (uint256 rng) {
    ...
    // Anchor block entropy to a block fixed at commit time, not one chosen at reveal time
    bytes32 anchor = blockhash(c.commitBlock + minRevealDelay); if (anchor == bytes32(0)) revert RevealWindowPassed();
    rng = uint256(keccak256(abi.encodePacked(expected, anchor))); emit Revealed(msg.sender, salt, secret, rng);
  }
...
Takeaway: bind and salt commitments; enforce windows; anchor block entropy to a predetermined block; clear state on success.
...
Commit–reveal offers simple, permissionless randomness with strong fairness properties, while VRF adds publicly verifiable proofs. Following the same principles as `zq2-random-number-commit-reveal`, you now have safe commit/reveal windows, binding, entropy anchored to a predetermined block, and unbiased selection you can deploy.
