...
4. **Initialize Hardhat 3 project:**
   ...
   ```
5. **Choose project configuration:**
...
const config: HardhatUserConfig = {
  solidity: {
    ...
    settings: {
      ...
      evmVersion: "shanghai", // Required for Zilliqa compatibility
    },
  },
  networks: {
    hardhat: {
...
```bash
rm contracts/Counter.sol contracts/Counter.t.sol
```
...
**Create a basic test file `test/MyFirstToken.test.ts`:**
...
describe("MyFirstToken", function () {
  it("Should deploy with correct initial state", async function () {
    ...
    const token = await MyFirstToken.deploy(claimant.address);
    
    // Check basic properties
    expect(await token.name()).to.equal("My First Token");
    ...
    expect(await token.claimant()).to.equal(claimant.address);
  ...
  it("Should transfer tokens correctly", async function () {
    const [owner, claimant, addr1] = await ethers.getSigners();
  ...
  it("Should reject zero address as claimant", async function () {
    ...
    await expect(
      MyFirstToken.deploy(ethers.ZeroAddress)
    ).to.be.revertedWith("Claimant cannot be zero address");
  });
});
```
...
4. 🔄 **Next: Testing and deployment** with Hardhat 3
