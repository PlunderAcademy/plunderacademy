...
Reading data is passive—users see their balances. Writing data is **active**—users take action! Transfers, mints, swaps, stakes—every blockchain interaction requires a signed transaction. We'll build a transfer form that sends tokens, tracks the transaction from signature to confirmation, and handles every edge case (user rejects, insufficient gas, reverts).
...
2. **Transaction lifecycle tracking** (signature → pending → confirmed)
...
3. MetaMask opens (user reviews transaction)
   ↓
4. User signs → transaction hash received
   ↓
...
export function TransferForm({ 
  ...
  return (
    <div className="rounded-xl border-2 border-primary/20 bg-card shadow-lg p-6">
      <h2 className="text-2xl font-bold mb-6 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Transfer Tokens</h2>
      ...
      <form onSubmit={handleTransfer} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder="0x..."
            disabled={isPending}
            className={`w-full rounded-lg border-2 px-4 py-3 text-sm font-mono transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30 ${
              errors.recipient 
          ...
          />
          {errors.recipient && (
            <p className="text-sm text-red-500 mt-1.5">{errors.recipient}</p>
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="0.0"
            disabled={isPending}
            className={`w-full rounded-lg border-2 px-4 py-3 text-base transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30 ${
              errors.amount 
                ? "border-red-500 bg-red-500/10" 
                : "border-border bg-background hover:border-primary/50"
            } disabled:opacity-50`}
          />
          {errors.amount && (
            <p className="text-sm text-red-500 mt-1.5">{errors.amount}</p>
          )}
        </div>
        ...
        <button
          ...
          className="mt-2 w-full rounded-xl bg-gradient-to-r from-primary via-purple-600 to-secondary px-8 py-4 text-lg font-bold text-white shadow-xl shadow-primary/30 hover:shadow-2xl hover:shadow-primary/40 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:from-gray-400 disabled:to-gray-500 disabled:shadow-none disabled:cursor-not-allowed disabled:hover:scale-100"
        >
          {isWritePending && "⏳ Awaiting signature..."}
          {isConfirming && "⏳ Confirming transaction..."}
          {!isPending && "🚀 Transfer Tokens"}
        </button>
      </form>
...
- **parseUnits**: Converts "1.5" → `1500000000000000000n` (with 18 decimals)
          ...
          <div className="space-y-8">
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Transfer ERC20 tokens, view your NFT collection, and explore the Zilliqa testnet—all with a beautiful, modern interface.
              </p>
            </div>
...
- [ ] Invalid address (e.g., "abc") → shows error
- [ ] Your own address → shows "Cannot send to yourself"
...
- [ ] Click "Try again" → error clears
- [ ] Can submit again

### 4. Insufficient balance
- [ ] Enter amount larger than your balance
- [ ] Click "Transfer" → approve in MetaMask
- [ ] Transaction reverts
...
3. User approves → transaction broadcast to network
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td><code>!isPending && !hash</code></td>
      <td>Nothing happening</td>
      <td>Show form</td>
    </tr>
    <tr>
      <td><strong>Awaiting signature</strong></td>
      <td><code>isWritePending</code></td>
      <td>MetaMask open, user deciding</td>
      <td>"Check your wallet"</td>
    </tr>
    <tr>
      <td><strong>Submitted</strong></td>
      <td><code>hash && isLoading</code></td>
      <td>Tx sent, waiting for block</td>
      <td>"Confirming..."</td>
    </tr>
    <tr>
      <td><strong>Confirmed</strong></td>
      <td><code>isSuccess</code></td>
      <td>Tx mined, receipt available</td>
      <td>"Success!"</td>
    </tr>
    <tr>
      <td><strong>Reverted</strong></td>
      <td><code>isError</code></td>
      <td>Tx failed on-chain</td>
      <td>"Transaction failed"</td>
    </tr>
    <tr>
      <td><strong>Rejected</strong></td>
      <td><code>writeError</code> (specific)</td>
      <td>User clicked "Reject"</td>
      <td>"You rejected"</td>
    </tr>
  </tbody>
</table>
...
- If not found, transaction wasn't broadcast → try again
...
async function checkTransfer() {
  try {
    ...
    // Simulation succeeded → safe to submit
  } catch (error) {
    // Simulation failed → show error before MetaMask opens
...
- ✅ Balance auto-refresh after transfer
- ✅ Explorer links for verification

**Next lesson (final):** We'll add polish—loading skeletons, success animations, comprehensive error mapping, and local testing strategies. We'll also discuss deployment options (Vercel, Netlify) and best practices for production.
...
**All checked?** Let's add the final polish! 🎨
