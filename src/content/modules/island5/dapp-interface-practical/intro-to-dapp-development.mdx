...
- ✅ **Connects wallets** with RainbowKit (MetaMask, WalletConnect, etc.)
- ✅ **Reads contract data** (ERC20 balances, NFT collections)
- ✅ **Writes to contracts** (token transfers with full transaction lifecycle)
- ✅ **Handles errors gracefully** (user rejections, reverts, network issues)
- ✅ **Manages network state** (Zilliqa testnet enforcement)
- ✅ **Provides great UX** (loading states, success feedback, clear messaging)

By the end, you'll have a **working dApp** that interacts with the contracts you deployed in previous islands—running locally and ready for production deployment.
...
<td>Wallet signature → transaction → wait for confirmation</td>
</tr>
<tr>
...
**Mental model:**
- **Traditional app**: Client ↔ Server ↔ Database
- **Web3 dApp**: Client ↔ Wallet ↔ Blockchain (the wallet IS the auth + signing layer)
...
**Core features:**
- ✅ **Wallet connection** (RainbowKit button in header)
- ✅ **Network enforcement** (must be on Zilliqa testnet)
- ✅ **Token balance display** (real-time, auto-refreshing)
- ✅ **Transfer interface** (send tokens to any address)
- ✅ **Transaction tracking** (pending → confirming → success)
- ✅ **NFT viewer** (display NFTs owned by connected wallet)
- ✅ **Error handling** (friendly messages for all edge cases)

**User flow:**
1. User visits dApp
2. Clicks "Connect Wallet" → RainbowKit modal appears
3. Selects MetaMask → connects and signs
4. dApp detects network → prompts switch to Zilliqa testnet if needed
5. Shows token balance and NFT collection
6. User enters recipient + amount → clicks "Transfer"
7. MetaMask opens → user confirms transaction
8. dApp shows "Pending..." → then "Success!" when confirmed
...
- Matches Plunder Academy's setup → you can reference the source code
...
│   ├── components/
│   │   ├── wallet-connect.tsx    # Connect button
│   │   ├── network-guard.tsx     # Network enforcement
│   │   ├── token-balance.tsx     # Read balance
│   │   ├── transfer-form.tsx     # Write transfers
│   │   └── nft-gallery.tsx       # Display NFTs
│   ├── lib/
│   │   ├── wagmi.ts              # Wagmi config (chains, transports)
│   │   └── utils.ts              # Helper functions (cn, etc.)
│   └── providers.tsx             # Wrap wagmi + RainbowKit + theme
...
- **Fresh project setup** (you'll create a new Next.js app, separate from your Solidity projects!)
...
<Callout type="info">
**Heads up:** In Lesson 5.6.2, you'll create a brand new **Next.js frontend project**—this is completely different from your Hardhat/Foundry Solidity projects! Make sure to close your previous project and start fresh in a new folder. Each practical module should be its own independent project!
</Callout>
...
### ⚠️ Important: Frontend Development Experience Required
...
**Required knowledge:**
- ✅ **React fundamentals**: Functional components, hooks (useState, useEffect, useContext), props, conditional rendering
- ✅ **TypeScript basics**: Types, interfaces, type assertions, generics
- ✅ **Next.js App Router**: Pages, layouts, client vs server components, routing
- ✅ **Async JavaScript**: Promises, async/await, error handling
- ✅ **Modern ES6+**: Arrow functions, destructuring, spread operator, template literals

**📚 Learning Resources (if you need to level up first):**
...
✅ **Completed Island 5, Lessons 5.1-5.5** (Web3 Frontend Basics)
...
- Transaction lifecycle: send → pending → mined → confirmed
