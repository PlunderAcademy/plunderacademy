...
- `accountStatus="address"` - Show abbreviated address
    ...
    <footer className="border-t border-border/40">
      <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between px-4 text-sm text-muted-foreground">
        ...
        <div className="flex gap-4">
          ...
          <a
            href="https://dev-wallet.zilliqa.com"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-foreground"
          >
...
- **Better layout**: Flexbox structure (header → main → footer)
...
Tailwind CSS v4 handles themes differently. Ensure your `src/app/globals.css` has theme variables:
...
4. **NetworkGuard:**
   - [ ] With wallet connected on Zilliqa testnet → content shows
   - [ ] Switch MetaMask to Ethereum → warning appears
   - [ ] Click "Switch to Zilliqa Testnet" → network changes
   - [ ] Content appears after successful switch

5. **Connection flow:**
   - [ ] Disconnected → shows "Connect your wallet"
   - [ ] Connected → shows address and green checkmark
...
**UX benefit:** Wallet connection always accessible. Users can disconnect, check address, or switch networks anytime—even if they scroll down.
...
No need to rebuild these for every page—reuse and compose!

## Next steps

You now have:
- ✅ Professional header with wallet connection
- ✅ Network enforcement (Zilliqa testnet only)
- ✅ Theme support (dark/light mode)
- ✅ Clean layout structure
- ✅ Reusable components

**Next lesson:** We'll read contract data—display ERC20 balances, view owned NFTs, and make the UI react to on-chain state. This is where your dApp starts feeling alive!
...
**All checked?** Let's read some contract data! 📖
