...
Your dApp is connected and network-enforced. Now let's make it useful by reading on-chain data! We'll fetch ERC20 token balances, display owned NFTs, and make the UI react to blockchain state. Reading data is the foundation of every dApp—it's how users see their holdings, positions, and activity.
...
> **Don't have contracts?** No problem! We'll provide testnet examples you can use.
...
# ERC20 Token Contract (use your own or this example)
NEXT_PUBLIC_TOKEN_ADDRESS=0x1234567890123456789012345678901234567890

# NFT Collection Contract (use your own or this example)
NEXT_PUBLIC_NFT_ADDRESS=0x0987654321098765432109876543210987654321
```

**Replace with your actual contract addresses** from previous islands, or use example addresses we'll provide.

### Create ABI files

Create `src/lib/abis/` directory:
...
We'll use **Option 1** (viem's built-in ABIs) for this lesson—works with any standard ERC20/721.
...
export function NFTGallery({ nftAddress, maxDisplay = 12 }: NFTGalleryProps) {
  ...
  // Fetch owned token IDs (simplified - just display count)
  useEffect(() => {
    if (balance && balance > 0n) {
      // In a real app, you'd query tokenOfOwnerByIndex or use an indexer
      // For now, just show the count
      const count = Number(balance);
      setOwnedTokens(Array.from({ length: Math.min(count, maxDisplay) }, (_, i) => i + 1));
    } else {
      setOwnedTokens([]);
    }
  }, [balance, maxDisplay]);
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {ownedTokens.map((tokenId) => (
              <div
                key={tokenId}
                className="aspect-square rounded-lg border border-border bg-muted/50 flex items-center justify-center hover:bg-muted transition-colors"
              >
                <div className="text-center">
                  <div className="text-2xl mb-1">🎨</div>
                  <p className="text-xs text-muted-foreground">#{tokenId}</p>
                </div>
              </div>
            ))}
          </div>
...
- **Simplified display**: Shows placeholders (real app would fetch metadata)
- **Empty state**: Friendly message when no NFTs owned
- **Responsive grid**: Adapts to screen size

> **Note:** A production NFT gallery would use `tokenOfOwnerByIndex` to get specific token IDs, then fetch metadata from `tokenURI`. We're keeping this simple for the lesson—you can enhance it later!
...
3. **Error handling:**
   - [ ] Invalid token address → error message appears
...
- ✅ NFT ownership viewing
- ✅ Loading states and error handling
- ✅ Auto-refreshing data
- ✅ Manual refresh buttons

**Next lesson:** We'll add the missing piece—**writing to contracts**! You'll build a transfer form that sends tokens, tracks transaction status, and updates the UI when confirmed. This is where wallets sign transactions and users see their actions go on-chain.
...
**All checked?** Time to write some transactions! ✍️
