...
You've built secure protocols with audits (smart-contract-audit-methodology), emergency mechanisms (emergency-response-circuit-breakers), and defense-in-depth (advanced-attack-vectors). But no system is perfect. When Cream Finance was exploited for $130M, the team didn't discover it until users reported missing funds—30 minutes too late. When Poly Network lost $611M, they detected it immediately via monitoring but had no escalation playbook—panic cost precious minutes.

**Frontend responsibility from previous modules**: When you built dApp interfaces with wallet connections, contract reading, and transaction management in earlier lessons, you learned to handle normal operations. But your frontend must also handle incident states: display clear "Protocol Paused" messages when contracts are frozen, provide UI for emergency withdrawals, show incident status banners, and prevent users from attempting transactions that will revert. Your monitoring alerts should trigger not just backend responses but also frontend status updates—users learn about incidents from your UI, not Twitter. Transparent error handling during crises builds trust and reduces support load.
...
**Definition — Incident**: Any security event requiring immediate attention, including: active exploits draining funds, discovered vulnerabilities (not yet exploited), oracle failures causing incorrect prices, admin key compromises, or abnormal user behavior patterns. **Why formal process matters**: Incidents trigger stress responses; training and playbooks override panic with effective action.

**Definition — Escalation tree**: Pre-defined list of contacts called in sequence during incidents. **Structure**: Level 1 = on-call engineer (immediate), Level 2 = security lead + CTO (if Level 1 can't resolve in 10 min), Level 3 = CEO + all founders (if critical or over $1M at risk), Level 4 = external advisors + legal (if over $10M or user data compromised). **Why tiered**: Prevents alert fatigue (not everyone for every issue) and ensures appropriate response level.

**Definition — War room**: Real-time coordination channel during active incidents. **Components**: Dedicated Discord/Slack channel, shared Google Doc for notes, video call for complex decisions, clearly assigned roles (not everyone talks at once). **Why structured**: Prevents duplicate work, missed actions, and communication confusion.

**Definition — Post-mortem**: Blameless retrospective analyzing incident cause, response effectiveness, and prevention measures. **Key sections**: Timeline, root cause, what went well, what didn't, action items with owners and deadlines. **Why blameless**: Encourages honesty; blame culture causes cover-ups.

**Mental model — Incident response phases**: Detect (monitoring finds anomaly) → Triage (assess severity) → Contain (stop damage spread) → Eradicate (fix vulnerability) → Recover (restore service) → Learn (post-mortem). Missing any phase causes issues (skip Contain = more loss, skip Learn = repeat incident).

**Anti-pattern — "We'll figure it out when it happens"**: No monitoring, no contacts list, no playbooks. **Why it fails**: First 5 minutes of an incident determine outcome. Figuring out "who do we call?" while funds drain wastes critical time. By the time you coordinate, attacker has moved funds to Tornado Cash.
...
# 1. Add Zilliqa 2.0 RPC in Tenderly dashboard (Settings → Networks)
...
app.post('/api/alerts/large-withdrawal', verifyTenderlyWebhook, async (req, res) => {
  ...
  const message = {
    embeds: [{
      title: '⚠️ LARGE WITHDRAWAL DETECTED',
    ...
    "panels": [
      ...
      },
      {
        ...
        "alert": {
          ...
          "message": "Share price changed by more than ±10%",
          "conditions": [{
            "type": "query",
...
- **P0 — Critical** → Funds actively draining; over $100K at risk
  - Response time: under 5 minutes
  - Escalation: page everyone
- **P1 — High** → Vulnerability discovered (not yet exploited); over $10K at risk
  - Response time: under 30 minutes
  - Escalation: security team + CTO
- **P2 — Medium** → Service degradation or abnormal activity
  - Response time: under 2 hours
  - Escalation: on-call engineer
- **P3 — Low** → Non‑critical issues or routine monitoring alerts
...
**Rotation & Fatigue Policy**:
- **Shift Length**: Max 6 hours during active war room. Cognitive decline leads to mistakes.
- **Handoff**: Outgoing IC must brief Incoming IC: "Status is X, next decision is Y."
- **Mandatory Sleep**: If incident > 24h, mandatory 8h sleep blocks for key personnel.
- **Vacation**: Designated backup must accept hand-off. No single point of failure.
...
**Fund Recovery Matrix**
| Scenario | Action | Example |
| :--- | :--- | :--- |
| **Whitehat (Communicating)** | Negotiate bounty (10-15%). | Poly Network |
| **Blackhat (Hostile)** | Trace funds, contact CEXs/Tether/Circle to freeze. | Lazarus Group hacks |
| **Loss < Treasury** | Cover from treasury immediately. | YieldVault (this lesson) |
| **Loss > Treasury** | Issue debt tokens (IOU) or compensation plan. | Badger DAO |
...
**Current Status**:
- ✅ Contract paused (14:38 UTC)
- ✅ Vulnerability patched (15:05 UTC)
- ✅ Contract unpaused and safe (15:16 UTC)
- ✅ All remaining user funds secure ($9.953M)

**Impact to Your Funds**:
- ✅ **Before 14:30 UTC**: Fully secure
- ⚠️ **Between 14:30-14:38 UTC**: Possible exposure (we're checking individual accounts)
- ✅ **After 14:38 UTC**: Fully secure
...
**Actions Taken**:
1. ✅ Vulnerability patched with ReentrancyGuard
2. ✅ Exploit test added to prevent regression
3. ✅ Emergency audit by Trail of Bits (started)
4. ✅ Bug bounty increased to $500K
...
### 7) Frontend incident handling

Your smart contracts might be paused, but if your frontend doesn't reflect that, users will panic when transactions fail.

#### Paused State Component

```tsx
// components/paused-overlay.tsx
...
export function PausedOverlay({ isPaused, incidentUrl }: PausedOverlayProps) {
  if (!isPaused) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="max-w-md rounded-lg border border-destructive/50 bg-destructive/10 p-6 shadow-lg">
        <div className="flex flex-col items-center text-center">
          <div className="mb-4 rounded-full bg-destructive/20 p-3">
            <AlertTriangle className="h-8 w-8 text-destructive" />
          </div>
          <h2 className="mb-2 text-xl font-bold text-foreground">Protocol Paused</h2>
          <p className="mb-4 text-muted-foreground">
            We have temporarily paused the protocol to investigate an anomaly. 
...
### 8) Mandatory Drills ("Game Days")

Playbooks are useless if your team is too panicked to read them. Schedule "Game Day" simulations quarterly.

#### Drill Scenarios
1.  **The Rug Pull**: Simulate a compromised admin key. Goal: execute emergency key rotation in < 15 mins.
2.  **The Drain**: Simulate a rapid TVL drop. Goal: detect and pause in < 5 mins.
3.  **The Oracle Failure**: Simulate bad price data. Goal: freeze specific markets without stopping entire protocol.

#### Drill Rules
*   **Surprise**: Only the "Game Master" knows the exact time (within a 1-week window).
*   **Production-like**: Use a fork of mainnet or a testnet mirror. NEVER drill on mainnet unless it's a read-only test.
*   **No Heroes**: The "expert" (e.g., CTO) should "die" (be unavailable) to test bus factor.

### 9) Decentralized Insurance & Liability

Don't be the only backstop for user funds. Integrate coverage.

*   **Nexus Mutual**: Purchase protocol cover or incentivize users to buy coverage.
*   **Sherlock**: Audit + coverage bundle.
*   **Risk Assessment**: Clearly label pools/vaults with risk scores so users know *their* liability vs. *protocol* liability.

### 10) Emergency Key Rotation

What if your deployer key is compromised?

1.  **Pre-signed Transactions**: Have a `transferOwnership` transaction pre-signed by the old key (if possible/safe) or ready to be signed by backup keys (multi-sig).
2.  **Hardware Wallet Backup**: Keep a backup ledger in a bank vault.
3.  **Procedure**:
    *   Identify compromise.
    *   Broadcast `revokeRole` or `transferOwnership` immediately from a safe machine.
    *   Move remaining admin funds to a fresh cold wallet.

### 11) Bug Bounty & VDP Management

Whitehats are your unpaid security team. Treat them well.

*   **Platform**: Use Immunefi or Cantina. Don't roll your own Google Form (legal/payment headaches).
*   **SLA**: Acknowledge reports in 24h. Decide payout in 7 days.
*   **Safe Harbor**: Explicitly state that whitehats won't be prosecuted if they follow rules.
*   **Payouts**: 10% of funds at risk is standard (cap at $1M-$5M usually).
...
contract IncidentResponseVault {
    ...
    // 🚨 CRITICAL: Missing access control on pause
    function pause() external {
        paused = true;
        emit Paused(msg.sender);
    }
    
    // 🚨 CRITICAL: Missing access control on unpause
    ...
    // 🚨 HIGH: Emergency withdraw doesn't work when paused
    ...
    // 🚨 CRITICAL: Admin can steal all funds during "emergency"
...
**Expected findings**:
- **CRITICAL**: `pause()` has no access control—any address can DoS the protocol
- **CRITICAL**: `unpause()` has no access control—attacker can unpause during incident response
- **CRITICAL**: `emergencyDrain()` allows admin to steal all user funds (no per-user withdrawals)
- **HIGH**: Emergency withdrawal impossible when paused—users cannot escape
 ...
 * @dev 🚨 Multiple upgrade safety issues
 */
contract IncidentResponseVaultV2 is Initializable, OwnableUpgradeable {
    ...
    // 🚨 HIGH: NEW storage variable added in V2 without gap
    ...
    // 🚨 CRITICAL: No timelock on emergency upgrade
    ...
    // 🚨 CRITICAL: Emergency withdrawal sends to centralized address
    ...
    // 🚨 HIGH: No validation on emergency recipient
...
**Expected findings**:
- **CRITICAL**: `emergencyUpgrade()` bypasses timelock—admin can rug pull with instant malicious upgrade
- **CRITICAL**: `emergencyWithdrawAll()` sends all funds to single address (not proportional per-user)
- **HIGH**: `setEmergencyRecipient()` has no validation—can be front-run or set to burn address
- **HIGH**: Missing storage gap for future upgrades—storage collision risk in V3
- **MEDIUM**: No event emission on `emergencyUpgrade()`—can't monitor upgrade history
...
## Severity Levels
- P0: Funds draining → Page everyone
- P1: Bug found → Call security team
- P2: Service slow → Email on-call
...
app.post('/api/alerts/large-withdrawal', async (req, res) => {
  ...
  await axios.post(DISCORD_WEBHOOK, {
    content: `⚠️ Large withdrawal: $${amount} from ${user}`
  });
...
- Monitoring covers ≥ 5 attack vectors (reentrancy, oracle, access control, economic, DoS)
- Alerts reach responders in < 60 seconds with full context
- Escalation has ≥ 2 levels with backup contacts and response time SLAs
...
**Containment**: Too late—funds already moved to Tornado Cash  
...
- [ ] Escalation tree documented (L1 → L2 → L3 → L4)
...
**What's next**: Take the Module 4 quiz to test your security knowledge, then apply these lessons to your own protocols. Security isn't a feature—it's a mindset. Build it into every line of code, every design decision, and every operational procedure.
