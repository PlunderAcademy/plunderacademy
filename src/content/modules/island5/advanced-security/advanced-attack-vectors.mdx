...
**Connection to previous lessons**: You've learned basic vulnerabilities (common-vulnerability-patterns), role-based access control for pausers and administrators (role-based-access-control), comprehensive testing including fuzz testing (security-testing-audit-preparation), and emergency response mechanisms (emergency-response-circuit-breakers). This lesson combines all those foundations with sophisticated attack patterns that bypass simple defenses—you'll need proper access control, thorough testing, and emergency circuits to defend against these advanced vectors.

This lesson teaches you to think like an advanced attacker: how flash loans provide risk-free capital for exploits, how to manipulate price oracles (and defend with TWAP + sanity checks), how cross-contract reentrancy bypasses simple guards, how storage collisions brick upgradeable contracts, how signature replay drains accounts, and how frontrunning extracts value. You'll learn defense in depth—layering controls so no single failure compromises security.

## Conceptual foundations for advanced attacks

**Definition — Flash loan attack**: Exploit using borrowed capital that's repaid in same transaction. **Why it's powerful**: Attacker needs zero capital, can borrow millions, exploit price/state assumptions that rely on capital constraints, repay instantly. If exploit fails, transaction reverts with no loss.

**Definition — Oracle manipulation**: Exploiting price feed vulnerabilities to borrow under-collateralized, liquidate unfairly, or drain liquidity pools. **Attack surfaces**: Low-liquidity AMM pools (manipulate via large swaps), stale oracle data (use outdated prices), centralized oracles (compromise data source), oracle front-running (exploit price lag).

**Definition — Cross-contract reentrancy**: Reentrancy via shared state across contracts. **Why guards fail**: Contract A has ReentrancyGuard, but attacker re-enters Contract B which shares state with A. Standard guard only checks single contract's lock.

<ReentrancyAttackVisualizer />

**Definition — Storage collision**: In upgradeable proxies, new implementation's storage layout conflicts with old, corrupting state. **Why it's critical**: Can brick contracts permanently, steal funds by reinterpreting storage slots, or grant attacker admin rights by overwriting access control slots.

**Definition — Signature replay attack**: Reusing valid signature on different contract/chain. **Attack vectors**: Signature lacks chainId (replay on forks), lacks contract address (replay on similar contracts), lacks nonce (replay multiple times), or lacks expiry (replay years later).

**Mental model — Economic attack modeling**: Instead of finding code bugs, model economic incentives. Ask: "Can attacker profit by exploiting this, even if code is 'correct'?" Flash loans, MEV, and oracle manipulation are economic attacks, not code bugs.

**Anti-pattern — "Our oracle is Chainlink, so we're safe"**: Assuming external dependency means security. **Why it fails**: Chainlink feeds can have stale data, low update frequency, or be de-listed. Must add circuit breakers, sanity checks, and fallback oracles.
 ...
 * @dev Uses spot price from AMM—can be manipulated in single transaction
 */
contract VulnerableLending {
    IERC20 public collateralToken;
    IERC20 public borrowToken;
    IUniswapV2Pair public priceOracle;  // ⚠️ Spot price = manipulable
     ...
     * @dev 🚨 VULNERABLE: Uses spot price which can be manipulated via flash loan
     */
     ...
     * @dev ⚠️ VULNERABILITY: Spot price can be manipulated in same transaction
     */
...
contract FlashLoanAttacker {
     ...
     *   2. Swap collateral → borrow tokens in AMM (manipulate price UP)
    ...
    function executeOperation(
        ...
        // 2. Swap 900K collateral → borrow tokens (manipulate price UP)
...
contract SecureLending {
    ...
    /**
     * @notice Borrow against collateral
     * @dev ✅ SECURE: Uses Chainlink + sanity checks + TWAP fallback
     */
    function borrow(uint256 amount) external {
...
**Layer 1: Chainlink oracle** → Prevents spot price manipulation
- Chainlink aggregates multiple sources, making manipulation difficult

**Layer 2: Freshness check** → Prevents stale data exploitation
- Rejects price data older than 1 hour

**Layer 3: TWAP comparison** → Detects sudden price changes
- TWAP cannot be manipulated in a single transaction

**Layer 4: Circuit breaker** → Halts operations during anomalies
...
- TWAP Required? ✅ Yes + formal verification
...
contract TokenVault is ReentrancyGuard {
     ...
     * @dev 🚨 BUT vulnerable to cross-contract reentrancy via rewardContract
     */
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // ⚠️ This can re-enter via RewardDistributor.claimRewards()
...
contract RewardDistributor is ReentrancyGuard {
     ...
     * @dev 🚨 VULNERABILITY: Reads vault.balances which may be inconsistent
     */
    function updateRewards(address user) external nonReentrant {
        // Read balance from vault
        // ⚠️ If called during vault.withdraw(), balance already decreased
...
contract SecureVault {
    ...
    function withdraw(uint256 amount) external crossContractNonReentrant {
        ...
        // ✅ Safe: rewardContract checks our reentrancyLock before executing
        rewardContract.updateRewards(msg.sender);
...
contract RewardDistributor {
    ...
    function updateRewards(address user) external {
        // ✅ Check if vault is currently executing (reentrancy detection)
...
- ✅ Multiple contracts share state (balances, rewards, deposits)
- ✅ Contract A calls Contract B which reads Contract A's state
- ✅ External calls happen during state transitions
- ✅ Protocol has over $100K TVL (economic incentive to exploit)

**Standard ReentrancyGuard is sufficient when:**
- ✅ Single contract, no external contract dependencies
- ✅ External calls only to trusted contracts (e.g., ERC20 transfers)
- ✅ All state updates complete before external calls (CEI pattern followed)
 ...
 * @dev 🚨 VULNERABILITY: Added variable BEFORE existing ones
 */
contract VaultV2 {
...
// Slot 0: bool paused (1 byte, but uses full 32-byte slot) ⚠️ COLLISION!
// Slot 1: address owner (now at wrong slot!) ⚠️ COLLISION!
// Slot 2: uint256 totalDeposits (now at wrong slot!) ⚠️ COLLISION!
// Slot 3: mapping(address => uint256) balances (now at wrong slot!) ⚠️ COLLISION!
```
...
/**
 * @title SecureVaultV2
 * @dev ✅ SECURE: New variables added at END using gap slots
 */
...
// ↑ This will throw error if storage layout conflicts detected
```
 ...
 * @dev 🚨 VULNERABILITY: Signature lacks nonce, chainId, expiry
 */
contract VulnerablePermit {
     ...
     * @dev 🚨 Can be replayed unlimited times
     * @dev 🚨 Can be replayed on forks (no chainId)
     * @dev 🚨 Can be replayed on similar contracts (no contract address)
     */
    function permit(
        address owner,
        address spender,
...
contract SecurePermit {
    mapping(address => mapping(address => uint256)) public allowance;
    mapping(address => uint256) public nonces;  // ✅ Prevents replay
     ...
     * @dev ✅ SECURE: Includes nonce (prevents replay), deadline (prevents stale), chainId, contract address
     */
    function permit(
        address owner,
        address spender,
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                ...
                spender,
                amount,
                nonces[owner]++,  // ✅ Increment nonce (prevents replay)
                deadline
            )
        );
...
**`nonce`** → Prevents multiple replays of same signature
- Each signature can only be used once

**`deadline`** → Prevents old signatures being reused years later
- Signatures expire after specified time

**`chainId`** → Prevents signature replay on forks (Polygon, BSC, etc.)
- Binds signature to specific blockchain

**`verifyingContract`** → Prevents signature replay on different contracts
- Binds signature to specific contract address

**`DOMAIN_SEPARATOR`** → Combines all protections into single hash
...
contract CommitRevealDEX {
     ...
     * @dev ✅ Prevents frontrunning because bot doesn't know amount until reveal
     */
    function reveal(uint256 amount, bytes32 secret) external payable {
        ...
        // Note: On Ethereum mainnet, 1 block ≈ 12 seconds
        // On Zilliqa 2.0, 1 block ≈ 1-2 seconds (faster finality)
...
contract SlippageProtectedDEX {
     ...
     * @dev ✅ Reverts if price moves beyond user's tolerance
    ...
    function buy(uint256 amount, uint256 maxPrice) external payable {
        ...
        // ✅ Revert if price moved too much
...
│  - Address checks, amount bounds, parameter sanitization   │
│  - Fast failure (low gas cost)                             │
└────────────────┬────────────────────────────────────────────┘
                 │ Passes
                 ▼
┌─────────────────────────────────────────────────────────────┐
│  Layer 2: BUSINESS LOGIC (Core functionality)               │
│  - Access control (onlyRole), state validations (require)   │
│  - Checks-Effects-Interactions pattern                      │
└────────────────┬────────────────────────────────────────────┘
                 │ Approved
                 ▼
┌─────────────────────────────────────────────────────────────┐
│  Layer 3: ORACLE SECURITY (Price/data integrity)            │
│  - Chainlink primary, TWAP backup, sanity checks            │
│  - Circuit breaker (auto-pause on anomaly)                  │
└────────────────┬────────────────────────────────────────────┘
                 │ Price valid
                 ▼
┌─────────────────────────────────────────────────────────────┐
│  Layer 4: REENTRANCY PROTECTION (Execution safety)          │
│  - ReentrancyGuard (single contract)                        │
│  - Cross-contract locks (shared state)                      │
│  - Pull over push pattern                                   │
└────────────────┬────────────────────────────────────────────┘
                 ...
                 ▼
┌─────────────────────────────────────────────────────────────┐
│  Layer 5: ECONOMIC SECURITY (Attack profitability)          │
│  - Collateralization ratios, liquidation mechanisms         │
│  - Position limits (prevent whale manipulation)             │
└────────────────┬────────────────────────────────────────────┘
                 │ Economically sound
                 ▼
┌─────────────────────────────────────────────────────────────┐
│  Layer 6: EMERGENCY CONTROLS (Last resort)                  │
│  - Pause mechanism, emergency withdrawal                    │
│  - Upgrade capability (fix vulnerabilities)                 │
└────────────────┬────────────────────────────────────────────┘
                 │ If attack detected
                 ▼
┌─────────────────────────────────────────────────────────────┐
│  Layer 7: MONITORING & RESPONSE (Detection/recovery)        │
│  - Event logging, anomaly detection                         │
│  - Incident playbooks (pre-planned responses)               │
└─────────────────────────────────────────────────────────────┘
...
- ❌ Single signature verification (no redundancy)
- ❌ No anomaly detection (120K ETH mint didn't trigger alerts)
- ❌ No pause mechanism on minting
- ❌ No mint limit (unlimited minting allowed)

**How defense in depth would have helped:**
1. ✅ Multi-signature requirement (3-of-5 guardians)
2. ✅ Mint limits ($10M max per transaction)
3. ✅ Timelock on large mints (24hr delay for over $1M)
4. ✅ Anomaly detection (auto-pause on unusual activity)
5. ✅ Circuit breaker (total mint cap per day)
...
**Defense in depth failures:**
- ❌ Single oracle source (Pyth with low liquidity)
- ❌ No sanity checks (allowed 1000x price change)
- ❌ No circuit breakers (didn't pause on extreme price movement)
- ❌ No borrow limits (allowed $110M single borrow)

**How defense in depth would have helped:**
1. ✅ Multiple oracle sources (Chainlink + Pyth + TWAP)
2. ✅ Price deviation limits (pause if over 50% change per hour)
3. ✅ Gradual position building (max $1M borrow per block)
4. ✅ Collateralization ceiling ($10M max borrow per user)
5. ✅ Time-weighted borrows (can't borrow full amount instantly)
...
1. **CRITICAL (10/10)**: Uses spot price from AMM—manipulable via flash loan
...
1. **CRITICAL (10/10)**: Signature lacks nonce—can be replayed unlimited times
...
2. **CRITICAL (10/10)**: Signature lacks chainId—can be replayed on forks/L2s
...
3. **CRITICAL (10/10)**: Signature lacks deadline—can be replayed years later
...
4. **CRITICAL (10/10)**: Signature lacks contract address—can be replayed on similar contracts
...
**Next lesson (smart-contract-audit-methodology)**: Smart Contract Audit Methodology—professional audit processes, preparing documentation for auditors, understanding audit firm selection, remediation procedures, and bug bounty program setup. You'll combine all attack vectors from this lesson into comprehensive audit preparation.
...
Advanced attacks require advanced defenses. Layer your security controls and model economic incentives—that's how billion-dollar protocols survive.
