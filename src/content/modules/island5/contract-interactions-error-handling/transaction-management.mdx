...
Reading data sets expectations; writing to contracts fulfills them. Transaction flows must be clear: prepare write, request wallet signature, show pending state, confirm success, and handle revert errors. We’ll use wagmi’s write + receipt hooks to manage the full lifecycle.

## Conceptual foundations for Web3 frontend

- **Definition — `useWriteContract`**: wagmi hook to call state‑changing functions. **When to use it**: Mints, transfers, staking—any write that changes storage.
- **Definition — `useWaitForTransactionReceipt`**: Track on‑chain confirmation. **When to use it**: After getting a transaction hash to reflect pending/success/failure.
- **Definition — Gas estimation**: Wallet/JSON‑RPC estimates cost to execute. **When to use it**: Pre‑flight checks; surface costs to users.
...
export function TransferToken({ token }: { token: `0x${string}` }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {isSigPending && <p className="text-sm">⏳ Waiting for wallet signature…</p>}
      {hash && (
        <p className="text-sm">
          📝 Tx submitted: <span className="font-mono text-xs">{hash}</span>
        </p>
      )}
      {isLoading && <p className="text-sm">⏳ Confirming on-chain…</p>}
      {isSuccess && <p className="text-sm text-green-600">✅ Success! Transaction confirmed.</p>}
      {writeError && <p className="text-sm text-red-600">❌ Error: {writeError.message}</p>}
      {isError && error && <p className="text-sm text-red-600">❌ Transaction reverted: {error.message}</p>}
    </div>
  );
}
```

Takeaway: Separate signature‑pending from chain confirmation; show hash early and final status when confirmed.

### 2) Gas and UX hints

Estimate gas pre‑flight where needed and hint at fees. Keep buttons disabled during pending states to prevent duplicates.
...
You can execute writes and reflect status cleanly. Next, you’ll add richer error handling and live event updates.
