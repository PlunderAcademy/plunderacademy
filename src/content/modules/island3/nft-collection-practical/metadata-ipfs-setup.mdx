...
**Before reveal:**
- `tokenURI(42)` → `"ipfs://QmHidden/hidden.json"`
- All tokens show same placeholder image
- Creates mystery and excitement

**After reveal:**
- `tokenURI(42)` → `"ipfs://QmRevealed/42.json"`
...
my-nft-collection/
├── metadata/
│   ├── hidden.json              # Placeholder metadata
│   ├── images/                  # Generated SVG files
│   │   ├── shape-001.svg
│   │   ├── shape-002.svg
│   │   └── ... (100 total)
│   ├── revealed/                # Per-token metadata
│   │   ├── 1.json
│   │   ├── 2.json
│   │   └── ... (100 total)
│   └── hidden-image.svg         # Placeholder image
└── scripts/
    └── generate-svgs.js         # Automated image generation
```
...
**Note:** The full script with all 100 shapes is [available here](/islands/island3/generate-svgs.js).
...
**Output:**
```
✅ Successfully created 100 SVG files in metadata/images/
```
...
console.log(`✅ Successfully created 100 metadata JSON files in ${revealedDir}/`);
console.log(`📝 Don't forget to replace IMAGES_CID in the files after uploading images to IPFS!`);
```
...
**Output:**
```
✅ Successfully created 100 metadata JSON files in metadata/revealed/
📝 Don't forget to replace IMAGES_CID in the files after uploading images to IPFS!
```
...
**Option 1: Manual update** `scripts/generate-metadata.js`:
```javascript
const imagesCID = 'QmXYZ123...abc'; // Your actual CID
```
...
async function main() {
  ...
  console.log("✅ Hidden URI updated!");
...
**IPFS Gateway URLs:**
- https://gateway.pinata.cloud/ipfs/QmHiddenCID/hidden.json
- https://ipfs.io/ipfs/QmHiddenCID/hidden.json
...
✅ Collection revealed successfully!
...
**Upload order:**
1. Images first → Get CID1
2. Update metadata with CID1
3. Upload metadata → Get CID2
...
✅ **Generated 100 unique SVG images** with automated script  
✅ **Created JSON metadata** for all tokens  
✅ **Uploaded to IPFS** using Pinata  
✅ **Updated contract URIs** with real IPFS links  
✅ **Tested reveal mechanics** (hidden → revealed)  
✅ **Verified in wallet** (MetaMask display)  
