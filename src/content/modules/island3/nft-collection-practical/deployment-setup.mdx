...
Visualize the journey of your smart contract from source code to live deployment on the Zilliqa network.
...
**Before you begin:** If you completed the Staking Contract module (Island 2) or any previous practicals, you should close that project and start with a completely new folder. Each practical module should be its own independent project!
</Callout>
...
├── my-nft-collection/     ← Your new Island 3 project (create this fresh!)
...
│   ├── reveal.ts
│   ├── interact.ts
│   └── generate-svgs.js        # Added in next lesson
├── metadata/                    # Added in next lesson
│   ├── hidden.json
│   ├── images/
│   └── revealed/
├── test/
  ...
  try {
    ...
    try {
      ...
      if (owner.toLowerCase() !== walletClient.account.address.toLowerCase()) {
        console.error("⚠️  Warning: Owner address mismatch!");
      }
    ...
    console.log("\n📋 Next Steps:");
    console.log("1. Test minting with mint-nft.ts script");
...
Create `scripts/mint-nft.ts` for minting NFTs:
...
async function main() {
  try {
    ...
    console.log("\n💰 Payment required:", payment.toString(), "wei");
    ...
    if (error.message?.includes("Exceeds max supply")) {
      ...
      console.log("\n💡 Not enough ZIL sent for minting");
    }
...
✅ NFT Collection deployed to: 0xNewNFTContractAddress
...
async function main() {
  ...
  if (owner.toLowerCase() !== walletClient.account.address.toLowerCase()) {
    console.error("\n❌ Only the owner can reveal the collection!");
    process.exit(1);
  }
  ...
  if (receipt.status === "success") {
    ...
    console.log("\n🎨 All NFTs now show their revealed metadata!");
    console.log("Transaction ID:", hash);
  } else {
...
✅ **Created the NFT contract** with claimant integration  
...
✅ **Tested minting** and tracking NFT ownership  
✅ **Created admin scripts** for reveal and interaction  
...
Ready to add metadata? Let's go! 🚀
