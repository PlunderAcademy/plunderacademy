...
describe("Staking - Reward Calculations", () => {
  ...
  for (const tc of testCases) {
    it(tc.name, async () => {
      const { staking, token, user } = await loadFixture(deployStakingFixture);
      ...
      // Advance time
      if (tc.days > 0n) {
        await time.increase(Number(tc.days) * 24 * 60 * 60);
      }
...
Every deposit emits a `Deposited(user, amount)` event. Writing `await expect(tx).to.emit(vault, "Deposited").withArgs(user, amount)` fifty times clutters your specs. Extract it once, use it everywhere, and gain consistency plus maintainability.
...
Takeaway: helpers reduce boilerplate and improve signal in specs. If the event signature changes (add a `positionId` parameter), you update one helper instead of grepping through 50 files.

### 3) AAA structure for deposit flow (Hardhat 3 + viem pattern)

**Explicit sections make setup and verification obvious, matching Island 1's tested pattern:**
...
```ts
// test/Staking.deposit.test.ts (matching Island 1 Hardhat 3 pattern)
import { loadFixture, time } from "@nomicfoundation/hardhat-toolbox-viem/network-helpers";
import { expect } from "chai";
import hre from "hardhat";

describe("Staking - Deposits", () => {
  async function deployStakingFixture() {
    const [owner, user] = await hre.viem.getWalletClients();
    const token = await hre.viem.deployContract("TestToken", []);
    const staking = await hre.viem.deployContract("Staking", [
  ...
  it("creates deposit with correct tier settings (Option 1: 30 days)", async () => {
    ...
    expect(apy).to.equal(10n);  // Tier 1 = 10% APY
    
    const duration = await staking.read.getStakedItemDuration([user.account.address, 0n]);
    expect(duration).to.equal(30n * 24n * 60n * 60n);  // 30 days in seconds
  });
});
```

**Takeaway**: Separating Arrange/Act/Assert with comments makes intent clear. This matches the **exact Hardhat 3 + viem pattern** from Island 1:
- `hre.viem.deployContract()` for deployment
- `hre.viem.getWalletClients()` for accounts
- `.write.functionName([args], { account })` for transactions
- `.read.functionName([args])` for view calls

This pattern is **tested and working** in Island 1's ERC20 module.
...
beforeEach(async () => {
  vault = await ethers.deployContract("StakingVault"); // Mutated by tests
});
```
...
```ts
beforeEach(async () => {
  return loadFixture(deployStakingFixture); // Fresh state every test
});
```

Fixtures snapshot initial state and restore it before each test. Every test gets a pristine contract, eliminating order dependencies.

**Rule**: Use fixtures or re-deploy; never share mutable contract instances.

**Bonus**: Fixtures are fast—Hardhat snapshots state at the EVM level, so "re-deploying" is instant. No performance penalty for isolation.
...
```ts
await vault.deposit(100);
await time.increase(86400); // Explicit 1 day
expect(await vault.previewReward(user)).to.equal(expectedReward);
```
...
**Rule**: Always manipulate time explicitly with `time.increase()` or `vm.warp()`.

**Tools**:
- **Hardhat**: `time.increase(seconds)` advances time relatively, `time.increaseTo(timestamp)` sets absolute time, `time.latest()` reads current block timestamp
- **Foundry**: `vm.warp(timestamp)` sets absolute time, `skip(seconds)` advances relatively, `block.timestamp` reads current time

**Testing multi-period scenarios**: For tests that verify reward accrual over multiple periods (deposit → 7 days → check → 30 days → check), use `time.increase()` between checks. This makes the test's timeline explicit:

```ts
await vault.deposit(1000);
await time.increase(7 * 86400); // 7 days
expect(await vault.previewReward(user)).to.equal(weekReward);
await time.increase(23 * 86400); // +23 days = 30 total
expect(await vault.previewReward(user)).to.equal(monthReward);
```
...
- [ ] **Custom helpers extracted**: Repeated event/state assertions moved to reusable helpers (DRY principle); update once when signatures change
- [ ] **Time control**: All time-dependent tests use explicit `time.increase()` or `vm.warp()`; no reliance on wall-clock or mining delays
- [ ] **Fixtures for isolation**: Tests use `loadFixture()` or equivalent; no shared mutable state between specs; every test gets fresh contracts
...
- [ ] **Test independence verified**: Run tests in random order; no failures indicate order dependencies; shuffle with `--bail` to catch leaky state
- [ ] **Fast feedback loop**: Unit tests run in under 5 seconds; split slow integration tests into separate suites; fixtures keep deployment fast
