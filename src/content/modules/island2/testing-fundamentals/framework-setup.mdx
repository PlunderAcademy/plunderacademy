...
```ts
// test/Token.fixture.ts (matching Island 1 Hardhat 3 + viem pattern)
...
Takeaway: fixtures cut suite time while keeping tests deterministic. `loadFixture()` snapshots state after `deployTokenFixture()` runs once, then restores that snapshot before each test. Adding 100 more tests using this fixture? Still only one deploy—the rest are instant resets.
...
```ts
// test/Staking.events.test.ts (matching Island 1 Hardhat 3 + viem pattern)
import { loadFixture } from "@nomicfoundation/hardhat-toolbox-viem/network-helpers";
import { expect } from "chai";
import hre from "hardhat";

describe("Staking events", function () {
  it("emits Deposit(address,uint256)", async function () {
    const { staking, token, user } = await loadFixture(async () => {
      const [owner, user] = await hre.viem.getWalletClients();
      const token = await hre.viem.deployContract("TestToken", []);
...
Takeaway: event assertions double as documentation for integrators. This test proves `Deposited` emits with the correct user address and amount—frontend developers can rely on this event structure. If the event signature changes, this test breaks immediately.
...
- **Fixture creation**: Write `deployStakingFixture()` that deploys vault + token, funds test accounts, and returns all contracts/signers
- **Baseline invariants**: In your fixture, assert initial invariants hold (vault balance = 0, totalStaked = 0, reward rate set correctly)
- **Event test**: Write a test that deposits to the vault and asserts `Deposited(user, amount)` emits with correct args
- **Custom error test**: Test `deposit(0)` and assert it reverts with `ZeroAmount()` custom error
- **Time manipulation**: Write a test that deposits, advances time by 30 days with `time.increase()`, then checks accrued rewards
...
**Pattern**: Use `loadFixture()` so every test gets a fresh snapshot. Never use global `let vault: StakingVault` that's deployed once and reused.

**Rule**: Fixtures should be stateless factories, not shared mutable objects.
...
### Inadequate fixture coverage

**Danger**: One fixture (`deployFixture`) used for all scenarios. Tests for "vault with 10 users and rewards accrued" spend 80% of their time setting up state that should be in a fixture.

**Pattern**: Create scenario-specific fixtures: `deployFreshVault()`, `deployVaultWithDeposits()`, `deployVaultWithAccruedRewards()`. Use the most relevant one per test.

**Benefit**: Tests run faster; setup intent is clearer.
...
- [ ] **Fixtures use loadFixture()**: All tests use `loadFixture()` for state isolation; no shared mutable contracts
- [ ] **Scenario-specific fixtures**: Multiple fixtures for common states (fresh vault, vault with deposits, vault with rewards)
- [ ] **Network helpers imported**: `time`, `loadFixture` imported from `@nomicfoundation/hardhat-toolbox-viem/network-helpers`
- [ ] **Chai matchers extended**: Hardhat's Chai matchers loaded for `.revertedWithCustomError()`, `.changeTokenBalance()`, etc.
- [ ] **Test file organization**: Tests grouped by contract; clear naming (`Staking.test.ts`, `Staking.deposits.test.ts`)
- [ ] **Helper directory**: `test/helpers/` exists for custom assertions; `test/fixtures/` for shared fixtures
- [ ] **Baseline invariant checks**: Fixtures assert initial invariants (totalStaked = 0, balances correct, roles assigned)
- [ ] **No hardcoded addresses**: Test accounts obtained via `hre.viem.getWalletClients()`; no dependency on specific addresses
- [ ] **Viem deployment pattern**: Use `hre.viem.deployContract()` and `hre.viem.getContractAt()` matching Island 1
- [ ] **Fast feedback**: Unit tests run in under 5 seconds; fixtures keep deployment time negligible

## Wrap‑up and next steps

You now have a production-aligned Hardhat 3 setup with fixtures for speed, network helpers for determinism, and assertion libraries for clear failures. This foundation—realistic compiler settings, efficient fixtures, organized file structure—scales from 10 tests to 1,000 without slowing down.
