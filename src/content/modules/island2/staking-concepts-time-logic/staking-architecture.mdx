...
🎯 **Unified Contract (What We'll Build in Module 2.5):**
...
🏗️ **Modular Architecture (Advanced Pattern - This Lesson):**
...
- **Starting out?** → Unified contract (simpler, proven, sufficient)
- **Large protocol with changing requirements?** → Consider modular
- **Team of 5+ developers?** → Modular helps separate concerns
- **Solo or small team?** → Unified is likely better

> 💡 **Learning Path:** In Module 2.5's practical exercise, we'll implement a unified staking contract to master the fundamentals. This lesson teaches the modular pattern as advanced knowledge—patterns you'll grow into as your protocols scale. Both approaches are valid and widely used in production.

## Where This Fits

Architecture is the difference between one‑off hacks and maintainable systems. By understanding both unified and modular patterns, you can choose the right approach for your scale and team.
...
In this lesson you'll learn modular boundaries that enable testing, upgrades, and integrations at scale. You'll see how stable events, interfaces, and pull‑based reward flows turn a staking system into a predictable platform others can build on. Then in Module 2.5, you'll apply these principles to build a production-ready unified contract.
...
Use thin interfaces (`IRewardPolicy`, `IRewardDistributor`) so you can swap implementations without redeploying or migrating storage. This is upgrade‑aware design: storage remains in one place, logic is replaceable by pointing to a new module address that satisfies the same interface.

### Event design is a public contract

Stable, well‑named events are how indexers and dashboards integrate. Treat event parameters as part of your API surface. Include `indexed` subjects like `user`, `positionId`, and capture enough context for analytics without leaking sensitive data.

### Pull > push for reward distribution

Favor pull‑based claims where users call `claim()` to receive their rewards. This isolates failures, simplifies accounting, and reduces gas spikes compared to pushing to many recipients. Distributors can still emit rich events so off‑chain systems stay current.

### Minimal, explicit admin controls

Administrative controls should be narrowly scoped: setting module addresses, adjusting policy parameters through the policy contract, and pausing only if essential. Prefer role‑based access with clear events for changes.

### Storage stability first

Plan data structures to survive future module swaps. Avoid packing fields you expect to grow; reserve space for future variables (when using upgradeable proxies). In non‑upgradeable cores, still design for migration—emit enough events to snapshot state if needed.
...
### 2) Compose flows with explicit events (analytics‑friendly)
...
### 4) RewardDistributor (pull‑based claims)
...
Takeaway: claims are isolated; failures don’t block others; events stay consistent.
...
<svg viewBox="0 0 720 260" xmlns="http://www.w3.org/2000/svg">
  ...
  <text className="label" x="370" y="92" textAnchor="middle">compute(amount, dt) → reward</text>
  ...
  <text className="label" x="365" y="195" textAnchor="middle">claim() → tokens</text>
</svg>

## Practice and reflection

- Sketch a diagram showing `StakingVault` → `RewardPolicy` → `RewardDistributor` data flow
...
**Danger**: Renaming or re‑ordering event fields silently breaks indexers.
...
**Rule**: Prefer pull‑based claims; batch only off‑chain.
...
**Good**:
```solidity
// policy is view‑only; core owns storage
function pending(address user) external view returns (uint128);
```
...
- [ ] **Pull claims**: No on‑chain loops pushing to many users
- [ ] **Interfaces only**: Modules referenced via thin interfaces
- [ ] **CEI discipline**: Checks → Effects → Interactions in value flows
...
By drawing clear lines between core, policy, and distributor, you get a system that’s easier to test, swap, and scale. You now have a blueprint for a modular staking system that’s predictable for indexers, safe to upgrade, and friendly to downstream integrations.

Next up, we’ll layer in time‑based logic—accurate accrual windows, locks, and vesting—so that your policy can compute rewards deterministically and your vault can enforce predictable exits. In Module 5, you’ll assemble these pieces into a complete, deployable staking contract and test suite (Zilliqa deployment considerations will be covered there).
