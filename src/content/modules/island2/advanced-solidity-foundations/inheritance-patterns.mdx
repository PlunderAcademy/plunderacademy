...
- **Checks-Effects-Interactions (CEI)**: Validate first, adjust internal state second, interact with external contracts last. This reduces reentrancy blast radius.
- **ReentrancyGuard**: Guard any function that moves tokens or calls untrusted code. Prefer pull payments (claim pattern) so external calls happen in isolated transactions.
- **Access control**: Use `Ownable` or `AccessControl` for privileged actions like pausing, updating policy, or emergency drains. Keep the surface tight and review role names.
- **Explicit override lists**: Always list every parent in `override(Parent1, Parent2)` when required. This is your audit breadcrumb.
- **Constructor and init ordering**: Base constructors run in linearized order, not in the order you call them. Keep constructors small; prefer immutables and explicit initialization.
- **Shadowing**: Avoid redeclaring variables with the same names as parents. It confuses readers and can lead to unintended reads/writes. Use distinct, descriptive names.
- **Ambiguous hooks**: In OpenZeppelin v5, prefer overriding the designated hook (e.g., `_update` for ERC20). Don’t sprinkle custom `require` checks across multiple surfaces unless you can prove they compose.
- **Events are for observers**: Emit events with `indexed` fields for client indexing, but never gate safety logic on event emission.
- **Approvals and token safety**: If interacting with ERC20s, prefer `SafeERC20` and avoid infinite approvals to third parties. Reset allowances when not needed.
- **Upgradability boundaries**: If you plan to upgrade policies, keep the boundary abstract and the storage layout stable. Never assume a child can read parent private storage.

### Review checklist before shipping

- **Have you minimized the virtual surface?** Only expose hooks you will actually override.
- **Is every override explicit?** If two parents mention the function, list them all.
- **Are `super` calls correct?** Ensure each layer runs exactly once and in the intended order.
- **Are cross-cutting rules centralized?** Pause checks and access control should sit in one predictable place.
- **Are events `indexed` appropriately?** Clients will thank you when building dashboards.
- **Have you written tests for the override chain?** Unit test each layer and an integration test that asserts end-to-end order.

## Wrap‑up and next steps

You learned how `virtual`/`override` create a clear chain of responsibility, how explicit overrides document intent in multiple inheritance, and how abstract contracts decouple the “what” from the “how.” You also practiced layering OpenZeppelin extensions with small, focused overrides like pause gates.

Next, we’ll deepen your grasp of data and errors. We’ll model staking-specific state, establish careful error handling, and prepare testable surfaces that make your reward policy and pause gates easy to verify.

## Assets (proposed)

- `/public/islands/island2/inheritance-patterns-diamond.svg`: Simple diamond diagram showing `Base`, `Left`, `Right`, `Diamond` with arrows indicating linearization order. **Alt**: “Diamond inheritance linearization order”.
- `/public/islands/island2/inheritance-patterns-policy.svg`: Block diagram of `StakingCore` depending on an abstract `RewardPolicy`, with swap-in `LinearAPRPolicy`. **Alt**: “Abstract policy plugged into staking core”.
- `/public/islands/island2/inheritance-patterns-pause.svg`: Sequence sketch of transfer attempt while paused, showing guard fail at `_update`. **Alt**: “Pause gate blocking token movement”.
