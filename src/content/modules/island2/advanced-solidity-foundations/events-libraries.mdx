...
**Why indexed parameters cost more:** When you mark a parameter as `indexed`, the EVM hashes the value using keccak256 and stores it as a separate **topic** in the log entry. Topics are stored in a way that enables efficient filtering by nodes and indexers. This hashing and separate storage costs approximately **375 gas per indexed parameter**, compared to ~8 gas per byte for non-indexed data. The trade-off: you pay upfront for efficient queries later. Index the fields you'll filter by (addresses, token IDs); leave amounts and timestamps non-indexed unless you specifically need to filter by exact values.
...
- **Over-indexing**: You can index up to 3 parameters per event. Indexing costs ~375 gas per topic because the EVM must hash the value (keccak256) and store it as a searchable topic. Don't index fields you'll never filter by (e.g., amounts, timestamps). Rule of thumb: index addresses and IDs; leave numeric values non-indexed.
- **Under-indexing**: If you don't index the primary subject (e.g., `account`), off-chain queries become slow (must scan all events). Without indexed topics, clients must download and decode every event of that type to find matches—expensive for high-volume contracts.
...
- **Delivery guarantees**: Events are not guaranteed to be delivered to off-chain systems in real-time or at all if your infrastructure fails. Node crashes, network partitions, or RPC rate limits can cause missed events. Production systems must implement retry logic, maintain event cursors (last processed block), and handle reorgs. Never assume an off-chain listener caught every event—design for "at-least-once" delivery with idempotent handlers. For critical operations, combine events with periodic state reconciliation (e.g., re-sync balances every hour from on-chain storage).
