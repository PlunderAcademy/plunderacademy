...
### ⚠️ CRITICAL: Understanding Centralization Risks

**This is a centralized staking contract.** Before deploying or using it, understand these risks:

**Owner Powers (Potential Risks):**
- 🟡 **Can stop rewards at any time** via `stopReward()` - users stop earning (but can still withdraw principal)
- 🟡 **Can change fee structures** via `updateFee()` - affects user withdrawals (capped at 10% max)
- 🟡 **Can change penalty rates** via `updateExitPenalty()` - affects early withdrawals (capped at 20% max)
- 🟡 **Can adjust APY rates** via `updateApy()` - only affects NEW deposits (existing deposits keep their rates)
...
- ✅ Transfer ownership to a multisig wallet (not single EOA)
- ✅ Add timelock delays for sensitive parameter changes
- ✅ This contract already has fee caps (10% withdrawal, 20% penalty) - good start!
- ✅ Consider progressive decentralization plan
- ✅ Regular third-party audits

**Safety Improvements in This Contract:**
- ✅ Withdrawal fees capped at 10% max
- ✅ Exit penalties capped at 20% max
- ✅ Zero address validation on deployment
- ✅ `availableRewards` logic protects user principal
- ✅ Transparent events for all parameter changes
...
- `totalStaked` → Tracks principal (doesn't include rewards)
- `feeAddress` → Can be changed by owner
- `claimant` → Your training portal wallet address (for achievement verification)
- APY values → Can be updated (but only affects NEW deposits)
...
**Why these events?**
- `indexed user` → Allows filtering by user address off-chain
...
- `tokenAddr` → Address of ERC20 token users will stake
- `feeAddr` → Where to send withdrawal fees (could be treasury, burn address, etc.)
- `_claimant` → Your training portal wallet address (for achievement verification)
...
**Manual calculation:**
```
Annual reward = 1000 tokens × 10% = 100 tokens per year
Daily reward = 100 ÷ 365 = 0.274 tokens per day
30-day reward = 0.274 × 30 = 8.22 tokens
```
...
Verification: 1000 × 0.10 × (30/365) = 8.219 ✓
```
...
1. ✅ Alice must have already approved the staking contract (just like with your ERC20 token!)
2. ✅ The contract transfers her tokens in
3. ✅ The contract creates a "receipt" tracking her deposit
4. ✅ The contract remembers: amount, time, and rate
...
3. User gets double credit! 💰💰 (bad!)
```

**WITH nonReentrant:**
```
1. First call to deposit() sets a "lock"
2. If deposit() tries to call itself → REVERTS
3. Lock releases when function completes ✅
```
...
So we measure: after - before = exactly what we received ✅
```
...
function transferFrom(address from, address to, uint256 amount) external {
    ...
    stakingContract.deposit(1000, 1);  // ← Blocked by nonReentrant
}
```
...
❌ You get your principal back MINUS a 5% penalty
❌ You get ZERO rewards
```

**Path B: Normal Withdrawal** (withdrawing after lock period ends)
```
You staked for 30 days, and 30+ days have passed
✅ You get your principal back MINUS a small 2% fee
✅ You get FULL rewards for the lock period
```
...
Penalty amount: 1000 × 0.05 = 50 tokens
...
Original deposit: 1000 tokens
Withdrawal fee: 2%
Fee amount: 1000 × 0.02 = 20 tokens
Rewards earned: 8.22 tokens (1000 × 10% × 30/365)
...
function _calculateReward(address _user, uint256 _index) internal view returns (uint256) {
    UserInfo storage user = userInfo[_user];
    if (startedTimestamp == 0 || user.amount == 0)  // ← Checks user.amount!
        return 0;
    // ... rest of calculation
}
```

If we calculate AFTER decrementing `user.amount`, the user gets 0 rewards! 💥
...
```
1. reward = _calculateReward(...);  // Uses current user.amount ✅
2. availableRewards = totalBalance - totalStaked;  // Uses current totalStaked ✅
3. user.amount -= amount;  // Now we can safely update state
4. totalStaked -= amount;
5. User gets correct rewards from correct pool! 🎉
```

**Checks-Effects-Interactions pattern (modified):**
1. ✅ Calculate values that depend on current state
2. ✅ Update state
3. ✅ Then do external calls
4. ✅ Prevents reentrancy and calculation bugs
...
User receives: 1000 - (1000 × 0.05) = 950 tokens
NO REWARDS PAID
```

**Safety checks:**
- `require(amount <= availableBalance)` → Ensures contract has enough balance
- `require(amount > 0)` → Prevents zero-value withdrawal (after penalty is applied)
...
- `if (reward > availableRewards)` → Cap rewards to what's actually available
- `require(withdrawAmount <= totalBalance - fee)` → Final sanity check
- `require(withdrawAmount > 0)` → Revert if something went wrong

**This design ensures:**
- ✅ Users ALWAYS get their principal back (minus fee)
- ✅ Rewards are paid from the surplus pool only
- ✅ Contract can't run out of tokens for withdrawals
...
FINAL:
Alice now has [DepA, DepD, DepC] ✅
Order changed, but that's okay! Each deposit is independent.
```

**Trade-offs:**
- ✅ **Much cheaper gas** (constant vs linear cost)
- ✅ **Scales better** (same cost for 1 or 100 deposits)
- ⚠️ **Order not preserved** (deposits may shuffle)
- ✅ **Order doesn't matter** (each deposit is independent!)
...
1. ✅ Deploy contract
2. ✅ Transfer reward tokens to the contract
3. ✅ Test with a small deposit
4. ✅ **Call `startReward()`** ← This turns on reward accrual!
...
You call stopReward()
Alice can still withdraw and get her 8.22 earned rewards ✅
She just won't earn any MORE rewards from this point ✅
```
...
```
Before: apy1 = 10
Alice deposits 1000 tokens → gets apy: 10

Owner calls updateApy(12, 18, 30)
Now: apy1 = 12

Bob deposits 1000 tokens → gets apy: 12
Alice still has apy: 10 (unchanged) ✅
```

**Safety features:**
- Max APY = 10,000% (100× return per year)
...
- ✅ Early exit penalty capped at **20% max**
- ✅ Withdrawal fee NOW capped at **10% max** (protects users!)
- ✅ Fee address validated (can't be zero address)
...
```solidity
// Owner tries:
updateFee(feeAddr, 99)  // ❌ REVERTS: "Fee cannot exceed 10%"

// Maximum possible:
updateFee(feeAddr, 10)  // ✅ Passes (10% max)
```
...
**State:** Alice's wallet → Staking contract can spend up to 1000 tokens
...
**Answer:**
```
reward = (5000 × (60 × 24 × 60 × 60) × 15) / 100 / (365 × 24 × 60 × 60)
       = (5000 × 5,184,000 × 15) / 100 / 31,536,000
       = 388,800,000,000 / 100 / 31,536,000
       = 123.287... tokens

Verification: 5000 × 0.15 × (60/365) = 123.287 ✓
```
...
2. Penalty: 10,000 × 0.05 = 500 tokens
...
**Answer:**
- Index 0: YES (35 days > 30-day lock) ✓
- Index 1: NO (20 days < 60-day lock) ✗
- Index 2: NO (5 days < 120-day lock) ✗
...
- ✅ `Ownable` for admin functions (`onlyOwner` modifier)
- ✅ Mappings to track user balances
- ✅ Transfer functions and approval patterns
- ✅ Events for tracking state changes
- ✅ Basic validation with `require()`

**What's New in Staking:**
- 🆕 **ReentrancyGuard** - Extra security for withdrawals
- 🆕 **Structs & Arrays** - Track multiple deposits per user (not just one balance!)
- 🆕 **Time Logic** - Calculate rewards based on `block.timestamp`
- 🆕 **Two Paths** - Different logic for early vs normal withdrawals
- 🆕 **SafeERC20** - Handle weird token edge cases
