...
- ✅ **Deposits work** for all duration tiers
- ✅ **Rewards calculate correctly** over time
- ✅ **Withdrawals handle** early vs normal cases properly
- ✅ **Penalties and fees** apply correctly
- ✅ **Edge cases** don't break the contract
- ✅ **Security** against reentrancy and other attacks
...
```
  Staking Contract
    ✔ Should deploy TestToken successfully (333ms)
    ✔ Should deploy Staking contract with correct initial state (571ms)
    ✔ Should allow owner to start rewards (363ms)
    ✔ Should not allow starting rewards twice (511ms)
    ✔ Should allow user to deposit (756ms)
    ✔ Should reject deposit with invalid option (501ms)
    ✔ Should reject deposit with zero amount (425ms)
    ✔ Should allow withdrawal after deposit (828ms)
    ✔ Should update APY correctly (393ms)
    ✔ Should reject APY above 10000% (341ms)
    ✔ Should handle multiple deposits correctly (756ms)
    ✔ Should calculate rewards correctly after 30 days (848ms)
    ✔ Should cap rewards at lock duration (751ms)
    ✔ Should apply penalty on early withdrawal (1055ms)
    ✔ Should pay rewards on normal withdrawal after maturity (1130ms)
    ✔ Should send withdrawal fee to fee address (1110ms)
    ✔ Should handle rewards starting after deposit (1127ms)
    ✔ Should handle withdrawal with swap-and-pop correctly (1252ms)
    ✔ Should allow owner to stop rewards (584ms)
    ✔ Should calculate rewardsRemaining correctly (769ms)
    ✔ Should allow owner to update exit penalty (406ms)
    ✔ Should reject exit penalty above 20% (378ms)
    ✔ Should reject updateFee with zero address (351ms)
    ✔ Should reject updateFee above 10% (410ms)
    ✔ Should handle rewardsRemaining when totalStaked exceeds balance (653ms)
    ✔ Should successfully update fee address and percentage (393ms)
    ✔ Should cap rewards when reward pool is insufficient (1023ms)
    ✔ Should return 0 reward when deposit made after rewards started but immediately checked (749ms)
    ✔ Should handle deposit time before reward start time (882ms)
...
✅ **Deployment tests** verify initialization with proper validation  
✅ **Deposit tests** cover all tiers (30/60/120 days) and edge cases  
✅ **Reward calculations** tested with time manipulation and capping logic  
✅ **Withdrawal tests** for early (penalty) and normal (rewards + fees) scenarios  
✅ **Multiple deposits** handled correctly with swap-and-pop optimization  
✅ **Admin functions** protected and validated (APY caps, fee caps, penalty caps)  
✅ **Security** against reentrancy, overflow, and reward pool exhaustion  
✅ **Edge cases** covered: rewards starting before/after deposits, insufficient reward pools, zero amounts  
✅ **Critical bug fixes** validated: calculation order dependencies

**What You've Achieved:**  
🔒 **Battle-Tested Security** - Comprehensive tests prove security measures work  
⚡ **Gas-Optimized** - Swap-and-pop array removal saves gas on withdrawals  
📊 **Full Coverage** - 98%+ coverage ensures no blind spots  
🛡️ **Fail-Safe Design** - Users always get their principal back, rewards are capped to available pool

**Congratulations!** You've built, tested, and deployed a staking contract on Zilliqa! 🎉
