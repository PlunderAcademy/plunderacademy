...
4. **Zilliqa testnet deployment** with verification
5. **Achievement claiming** using your transaction hash
...
**Before you begin:** If you completed the ERC-20 Token module (Island 1), you should close that project and start with a completely new folder. Each practical module should be its own independent project!
</Callout>
...
2. **Choose a clean location for the new project:**
   ...
   - Do NOT create this inside your previous `my-zilliqa-token` folder!
...
```
Documents/
├── my-zilliqa-token/      ← Your Island 1 ERC-20 project (keep it here!)
├── my-staking-contract/   ← Your new Island 2 project (create this fresh!)
...
# Open this fresh project in VS Code
code .
```

<Callout type="info">
**Opening in VS Code:** The `code .` command opens the current folder in a new VS Code window. If VS Code doesn't open, you can manually open VS Code and use File → Open Folder to select `my-staking-contract`.
</Callout>
...
my-staking-contract/
├── contracts/
│   └── Staking.sol          # Your staking contract
├── scripts/
│   ├── deploy-staking.ts    # Deployment script
│   └── interact.ts          # Interaction script
├── test/
│   └── Staking.test.ts      # Tests (Module 2.5.4)
...
- `address public claimant` → Your training portal wallet
...
async function main() {
  try {
    ...
    if (!stakingTokenAddress) {
      console.error("❌ STAKING_TOKEN_ADDRESS not set in .env");
    ...
    console.log("✅ Staking contract deployed to:", contractAddress);
    ...
    try {
      ...
      if (stakingToken.toLowerCase() !== stakingTokenAddress.toLowerCase()) {
        console.error("⚠️  Warning: Staking token mismatch!");
      }
...
### Creating a Test Token (Optional)

If you don't already have an ERC20 token from Island 1, you can create a simple test token to stake.
...
async function main() {
  try {
    ...
    const contractAddress = receipt.contractAddress!;
    console.log("✅ TestToken contract deployed to:", contractAddress);
    ...
    console.log("\n📋 Use this address as STAKING_TOKEN_ADDRESS in your .env file");
...
✅ Staking contract deployed to: 0xNewStakingContractAddress
...
async function main() {
  try {
    ...
    if (!stakingContractAddress) {
      console.error("❌ STAKING_CONTRACT_ADDRESS not set in .env");
...
async function main() {
  try {
    ...
    console.log("\n📊 Current staking contract status:");
    ...
    console.log("Rewards started:", startedTimestamp.toString() === "0" ? "❌ No" : `✅ Yes (at timestamp ${startedTimestamp})`);
    console.log("Total staked:", totalStaked.toString());
    console.log("Rewards available:", rewardsRemaining.toString());
    console.log("APY Tier 1 (30 days):", apy1.toString() + "%");
    console.log("APY Tier 2 (60 days):", apy2.toString() + "%");
    ...
    if (startedTimestamp.toString() !== "0") {
      console.log("\n⚠️  Rewards have already been started!");
    ...
    if (rewardsRemaining === 0n) {
      console.log("\n⚠️  Warning: No rewards available in the contract!");
    ...
    if (receipt.status === "success") {
      console.log("✅ Rewards started successfully!");
      ...
      console.log("\n📊 Updated staking contract status:");
    ...
    if (error.message?.includes("Can only start rewards once")) {
      console.log("\n💡 Rewards have already been started for this contract.");
    } else if (error.message?.includes("OwnableUnauthorizedAccount")) {
      console.log("\n💡 Only the contract owner can start rewards.");
...
async function main() {
  try {
    ...
    if (![1, 2, 3].includes(option)) {
      console.error("❌ Invalid DEPOSIT_OPTION. Must be 1, 2, or 3");
      process.exit(1);
    }
    ...
    console.log("Rewards started:", startedTimestamp.toString() === "0" ? "❌ No" : "✅ Yes");
    console.log("Total staked:", totalStaked.toString());
    console.log("Rewards available:", rewardsRemaining.toString());
    console.log("\nAPY Options:");
    ...
    console.log("\n✅ Selected: Option", option, "-", selectedApy.toString() + "% APY for", selectedDuration);
    
    if (startedTimestamp.toString() === "0") {
      console.log("\n⚠️  Warning: Rewards haven't started yet!");
    ...
    if (currentAllowance < amount) {
      ...
      if (approveReceipt.status === "success") {
        ...
        if (updatedAllowance < amount) {
          console.error("❌ Allowance verification failed. Please try again.");
    ...
    console.log("\n🔍 Final verification before deposit...");
    ...
    console.log("\n📥 Depositing to staking contract...");
    ...
    if (error.message?.includes("Invalid Amount")) {
      ...
      console.log("\n💡 Deposit option must be 1, 2, or 3");
    } else if (error.message?.includes("insufficient allowance")) {
      console.log("\n💡 Token approval failed or insufficient");
    }
...
✅ **Created the Staking contract** with claimant integration  
...
✅ **Funded the reward pool** with tokens  
✅ **Started reward accrual** as contract owner  
✅ **Made test deposits** and tracked staking positions  
✅ **Queried contract information** for monitoring  
