...
**Before starting this module, you must complete the Development Environment Setup** from Module 4.3. This includes:
- WSL2 installation (Windows users - REQUIRED)
- VS Code setup with WSL extension
- Node.js 22.10+ installation
- Basic Hardhat 3 project configuration

If you haven't completed the environment setup, please go back to Module 4.3 first. The "Professional Development" path in this module requires that setup.

---

## 💾 Important: Saving Files in VS Code

**Throughout this module, you MUST save your files after editing them!**

### How to Know if a File Needs Saving:
- **White dot** next to the filename in the tab = file is NOT saved
- **X** next to the filename = file is saved

### Ways to Save:
- **Single file:** Press **Ctrl+S** (Windows/Linux) or **Cmd+S** (Mac)
- **All files:** Press **Ctrl+K** then **S**, or use **File → Save All**
- **Auto-save:** Enable it in VS Code settings: File → Preferences → Settings → search for "Auto Save"

### ⚠️ Common Mistakes:
- Compiling without saving contract files = old code gets compiled
- Running scripts without saving = old scripts run
- Editing `.env` without saving = deployment fails with wrong credentials

**Pro tip:** Use **File → Save All** before running any command in the terminal!
...
🚀 **Quick Creation (Recommended):** Use our Token Factory
- ✅ Create tokens in minutes with just a few clicks
- ✅ No development setup required
- ✅ Works with any wallet (mobile-friendly)
- ✅ Perfect for learning token fundamentals

🛠️ **Professional Development:** Full Hardhat Setup
- ✅ Complete development environment
- ✅ Learn professional deployment workflows
- ✅ Understand contract compilation and testing
...
**Factory Benefits:**
- ✅ **Gas efficient:** ~50% less gas than individual deployment
- ✅ **Battle-tested:** Uses proven OpenZeppelin patterns
- ✅ **Instant verification:** Automatic claimant tracking
- ✅ **Mobile-friendly:** Works with any Web3 wallet
- ✅ **No coding required:** Pure wallet interaction
...
**Real-world analogy:** Learning to build tokens is like learning to drive. You don't start with a Formula 1 race car—you begin with a reliable car that gets you from point A to point B safely. Once you master the basics, you can add advanced features.

**Our approach:**
1. ✅ **Simple token first:** Just 10 lines of code that works
2. ✅ **Understand every line:** Know exactly what your code does
3. ✅ **Deploy and test:** Get hands-on experience
4. ✅ **Add features gradually:** Learn professional patterns step by step
...
1. **Open your terminal:**
   - **Windows users:** Open VS Code, then open terminal (View → Terminal or Ctrl+`), type `wsl` to enter your bash prompt with the `$` symbol
   - **Mac/Linux users:** Use your regular terminal
2. **Navigate to your development directory** (run in WSL bash for Windows):
   ```bash
...
3. **Create a new project:**
   ...
   cd my-first-token
   ```
4. **Initialize Hardhat 3 project** (run these commands in your WSL bash terminal):
   ```bash
   npm init -y
...
5. **Choose the following when prompted:**
   ...
   - ✅ **Project root:** Press Enter to accept the default (current directory) - **DO NOT type the folder name**
...
**Add the OpenZeppelin library to your project** (run this command in your WSL bash terminal):

```bash
npm install @openzeppelin/contracts
```

OpenZeppelin provides battle-tested, secure implementations that are used by thousands of projects. Think of it as your security foundation—never reinvent critical security code when proven solutions exist.

### Project Configuration

**Update your `hardhat.config.ts` for Zilliqa:**

⚠️ **IMPORTANT:** Replace the ENTIRE contents of the existing `hardhat.config.ts` file with the code below. Delete everything currently in the file and paste this complete configuration:
...
- Press **Ctrl+S** (Windows/Linux) or **Cmd+S** (Mac) to save `hardhat.config.ts`
- Look for the white dot next to the file name in VS Code - if you see it, the file is NOT saved
- You can also use **File → Save** from the menu
...
```
ZILLIQA_TESTNET_PRIVATE_KEY=0x_your_testnet_private_key

ZILLIQA_MAINNET_PRIVATE_KEY=0x_your_production_key_here_when_ready

CLAIMANT_ADDRESS=0x_your_training_wallet_address
...
⚠️ **Important notes about the .env file:**
- Replace the placeholder values with your actual wallet information when you have it
- The `0x` prefix is required for private keys and addresses
- Never share this file or commit it to Git
- Keep this file in your project root directory

### 🚨 CRITICAL: Understanding the Difference Between Private Keys and Public Addresses

**CLAIMANT_ADDRESS = Your PUBLIC wallet address (42 characters):**
- ✅ This is **safe to share** and visible to everyone on the blockchain
- ✅ Copy it from MetaMask by clicking your account name
- ✅ Example: `0x47830f1be175a7f8b2906eff6e252b5d4fa3cc91` (42 chars)

**ZILLIQA_TESTNET_PRIVATE_KEY = Your PRIVATE key (66 characters):**
- ❌ This is **SECRET** - never share it with anyone
- ❌ Much longer than an address (66 chars vs 42 chars)
- ❌ Example: `0xa1b2c3d4e5f6789012345678901234567890abcdef1234567890abcdef123456` (66 chars)

**⚠️ DO NOT put your private key in the CLAIMANT_ADDRESS field!** If you do, your wallet is compromised.

💾 **CRITICAL: SAVE THE FILE!** 
- Press **Ctrl+S** (Windows/Linux) or **Cmd+S** (Mac) to save `.env`
- Verify the file is saved by checking for the white dot indicator in VS Code

**Install additional dependencies** (run these commands in your WSL bash terminal):
```bash
...
**Remove the default Counter contract** (run this command in your WSL bash terminal):
```bash
...
💾 **CRITICAL: SAVE THE FILE!** 
- Press **Ctrl+S** (Windows/Linux) or **Cmd+S** (Mac) to save `MyFirstToken.sol`
- **If you don't save, the compile command will fail!**

**Just 12 lines of functional code with verification support!**

This token can:
- ✅ **Transfer tokens** between addresses
- ✅ **Show balances** in wallets
- ✅ **Be traded** on exchanges
- ✅ **Approve spending** for other contracts
- ✅ **Work with all ERC-20 compatible tools**
...
💾 **BEFORE YOU COMPILE:** Make sure ALL files are saved!
- Look at the file tabs in VS Code - **white dots mean unsaved files**
- Use **File → Save All** or press **Ctrl+K** then **S** to save everything
- Compiling unsaved files will fail or use old code

**Compile your contract** (run this command in your WSL bash terminal):
...
💾 **CRITICAL: SAVE YOUR TEST FILES!** 
- If you created `test/MyFirstToken.test.ts`, save it: **Ctrl+S** (Windows/Linux) or **Cmd+S** (Mac)
- If you created `contracts/MyFirstToken.t.sol`, save it too
- Or use **File → Save All** (Ctrl+K S) to save all open files at once
- **Unsaved test files will not be found when you run tests!**
...
**Run your tests** (run this command in your WSL bash terminal):
...
Running Solidity tests
  ...
  contracts/MyFirstToken.t.sol:MyFirstTokenTest
    ✔ test_ZeroAddressClaimant()
    ✔ test_TransferMoreThanBalance()
    ✔ test_Transfer()
    ✔ test_InitialState()
    ✔ test_ClaimantIsImmutable()
    ✔ testFuzz_Transfer(uint256) (runs: 256)
...
Running node:test tests
  ...
  MyFirstToken
    ✔ Should deploy with correct initial state (210ms)
    ✔ Should transfer tokens correctly (183ms)
    ✔ Should reject zero address as claimant (143ms)
...
- **Growing project?** → Add owner controls
- **Need scarcity?** → Add supply cap  
- **Security concerns?** → Add pause functionality
- **Multiple admins?** → Add role-based access control
...
**Your learning path:**
1. ✅ **Created simple token** with OpenZeppelin
2. ✅ **Understood every line** of code
3. ✅ **Learned professional features** available
4. 🔄 **Next: Deployment** with Hardhat 3
5. 🔄 **Then: Integration and management** best practices
