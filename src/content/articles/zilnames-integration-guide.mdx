...
- ✅ **Forward Resolution**: Convert `name.zil` → `0x...address`
- ✅ **Reverse Resolution**: Convert `0x...address` → `name.zil`
- ✅ **Avatar Support**: Display profile images
- ✅ **Text Records**: Store additional metadata (URLs, descriptions, etc.)
- ✅ **Full ENS Compatibility**: Works with Viem's built-in ENS functions
- ✅ **Cross-App Support**: Names work across the entire Zilliqa ecosystem
    ...
    <div className="container mx-auto max-w-4xl p-6">
      ...
      <section className="mt-8 p-6 bg-muted rounded-lg">
        ...
        <ul className="space-y-2 text-sm text-muted-foreground">
          <li>• Forward resolution works for any registered .zil name</li>
          <li>• Reverse resolution only works if the owner has set a primary name</li>
          <li>• Results are cached for 5 minutes to improve performance</li>
          <li>• Register your own .zil name at <a href="https://zilnames.com" target="_blank" rel="noopener" className="text-primary hover:underline">zilnames.com</a></li>
        </ul>
      </section>
    </div>
  )
...
|----------|---------|---------|
| `getEnsAddress(name)` | Name → Address | `Address \| null` |
| `getEnsName(address)` | Address → Name | `string \| null` |
| `getEnsAvatar(name)` | Get avatar URL | `string \| null` |
| `getEnsText(name, key)` | Get text record | `string \| null` |
| `getEnsResolver(name)` | Get resolver address | `Address \| null` |
