...
├── unit/
│   ├── TreasureChest.t.sol
│   └── TreasureChestFuzz.t.sol
├── integration/
│   └── TreasureChestIntegration.t.sol
├── invariant/
│   └── TreasureChestInvariant.t.sol
└── mocks/
    └── MockERC20.sol
```
...
contract TreasureChestTest is Test {
    ...
    function setUp() public {
        vm.prank(captain);
        chest = new TreasureChest{value: 1 ether}(); // 1 ETH initial deposit
...
contract TreasureChestUnitTest is TreasureChestTest {
    function testInitialState() public {
        ...
        assertEq(chest.getTotalDeposits(), 1 ether);
    ...
    function testDepositGold() public {
        ...
        assertEq(chest.getTotalDeposits(), 3 ether); // 1 + 2
        assertEq(chest.contributions(crew1), depositAmount);
        assertEq(chest.getBalance(), 3 ether);
    }
    ...
    function testWithdrawGold() public {
        ...
        assertEq(chest.getTotalDeposits(), 1.5 ether); // 3 - 1.5
...
contract TreasureChestAccessTest is TreasureChestTest {
    function testOnlyCaptainCanWithdraw() public {
        ...
        vm.prank(pirate);
        vm.expectRevert(abi.encodeWithSignature("OnlyCaptain()"));
...
contract TreasureChestFuzzTest is TreasureChestTest {
    function testFuzzDepositAlwaysIncreasesGold(uint256 amount) public {
        ...
        uint256 totalDepositsBefore = chest.getTotalDeposits();
        ...
        assertEq(chest.getTotalDeposits(), totalDepositsBefore + amount);
    ...
    function testFuzzWithdrawAlwaysDecreasesGold(uint256 depositAmount, uint256 withdrawAmount) public {
        ...
        assertEq(chest.getTotalDeposits(), totalDepositsBefore - withdrawAmount);
    }
}
```
...
function testZilliqaPerformance() public {
    ...
    assertEq(chest.getTotalDeposits(), 1 ether + (numOperations * 0.001 ether));
}
```
...
function testDebugExample() public {
    console.log("Total deposits before:", chest.getTotalDeposits());
    
    vm.prank(crew1);
    chest.depositGold{value: 1 ether}();
    
    console.log("Total deposits after:", chest.getTotalDeposits());
...
Set sail with confidence, knowing your code is battle-tested! ⚓️🧪