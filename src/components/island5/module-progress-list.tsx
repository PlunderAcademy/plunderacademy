...
export function NeonModuleProgressList({
  ...
  const progressState = useMemo(() => {
    ...
    const neonAchievementNumbers = NEON_MODULES
      .map(slug => MODULE_TO_ACHIEVEMENT_MAP[slug])
      .filter((value): value is string => Boolean(value));

    walletAchievements.forEach(achievement => {
      if (!achievement.isClaimed) return;
      const index = neonAchievementNumbers.indexOf(achievement.achievementNumber);
      if (index !== -1) {
        completed.add(index);
      }
    });
      ...
      <div className="space-y-4">
        {orderedModules.map((module, index) => {
          ...
          return (
            <Link
              key={module.slug}
              to="/lessons/island5/$module" params={{ module: module.slug }}
